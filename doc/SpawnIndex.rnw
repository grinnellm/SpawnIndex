...
\newcommand{\trAuthsLong}{Matthew H.~Grinnell$^1$, Jake F.~Schweigert$^2$, Matt Thompson$^1$, and Jaclyn S.~Cleary$^1$}

% Author names without
\newcommand{\trAuthsNoMath}{Matthew H.~Grinnell, Jake F.~Schweigert, Matt Thompson, and Jaclyn S.~Cleary}

% Author names reversed (for citation)
\newcommand{\trAuthsBack}{Grinnell, M.H., Schweigert, J.F., Thompson, M., and Cleary, J.S.}
...
First, we develop a sampling protocol to estimate the number of eggs in a given area.
...
Premièrement, nous élaborons un protocole d'échantillonnage pour estimer le nombre d'oeufs dans une zone donnée.
...
extSA <- shapes$saDF %>%
  filter( id==zoomSA ) %>%
  group_by( id ) %>%
  summarise( xmin=min(Eastings)-bufSA, xmax=max(Eastings)+bufSA,
...
extSec <- shapes$secDF %>%
  filter( id==zoomSec ) %>%
  group_by( id ) %>%
  summarise( xmin=min(Eastings)-bufSec, xmax=max(Eastings)+bufSec,
...
BaseMap <- ggplot( data=shapes$landCropDF, aes(x=Eastings, y=Northings) ) +
  geom_polygon( data=shapes$landCropDF, aes(group=group), fill="darkgrey" ) +
...
intensity <- intensity %>%
  rename( f1969t1978=Category ) %>%
  mutate( f1928t1968=c(1, NA, 2, NA, 3, NA, 4, NA, 5) ) %>%
  select( f1928t1968, f1969t1978, Description, Layers ) %>%
  arrange( Layers )
...
For \fishName{} (\scienceName{}) in British Columbia (BC), Canada, an index of relative population abundance is provided by monitoring the extent and intensity of spawn (i.e., egg) deposition throughout coastal BC \citeyearpar[DFO][]{DFO2019a}.
This document describes our calculations to convert spawn survey observations (e.g., spawn extent, number of egg layers, substrate type) to the spawn index for \fishName{} in BC.

\cite{HartTester1934} first demonstrated that an estimate of \fishName{} abundance could be determined from a count of egg deposition in a small set of sampling quadrats.
The calculations in this document have been described elsewhere, in either published or informal, internal documents.
The objective of this document is to collate the various calculations in their order of application.
...
Annual monitoring surveys of egg deposition collect data used to calculate the spawn index.
There are three types of spawn survey observations:
\begin{enumerate}
...
For example, surface spawn surveys were the only survey type prior to 1988, and they are still used extensively for minor spawns, remote spawns (i.e., outside stock assessment region boundaries; see below), as well as unusually early or late spawns.
Macrocystis and understory spawn surveys are conducted using SCUBA gear, and have been used for all major spawns since 1988.
Thus, we describe the spawn index as having two periods based on the predominant survey type:
the surface survey period from 1951 to 1987, and
the dive survey period from 1988 to present.

The inclusion of dive surveys in 1988 makes it challenging to compare the spawn index between these two periods.
For example, surface surveys are less accurate than dive surveys (\hyperref[secWidthAdjust]{appendix \ref{secWidthSurface}}), and
spawn surveyors used subjective intensity categories instead of direct egg layer estimates until 1981 (\autoref{secIntensity}).
In addition, \fishName{} spawn survey effort has been inconsistent over the time series due to available resources and departmental priorities.
...
\fishName{} spawn surveys began in 1928, but are considered incomplete for indexing purposes prior to 1937 because many potential areas were not surveyed \citep{HayKronlund1987}.
...
There are \Sexpr{Num2Word(nrow(regions))} SARs in BC (\autoref{figBC}; \citealt{HaistRosenfeld1988}).
The major SARs are Haida Gwaii (formerly known as Queen Charlotte Islands), Prince Rupert District, Central Coast, Strait of Georgia, and West Coast of Vancouver Island.
The minor SARs are
...
# Plot Section
mapSec <- BaseMap +
  geom_path( data=filter(shapes$secDF, id%in%zoomSec), aes(group=Section),
    size=0.25, colour="black" ) +
  coord_cartesian( xlim=c(extSec$xmin, extSec$xmax),
    ylim=c(extSec$ymin, extSec$ymax), default=TRUE ) +
  geom_point( data=spawnByLocXY, aes(colour=TotalSI), size=3, alpha=0.5 ) +
  scale_colour_viridis( labels=comma ) +
  labs( colour="Spawn\nindex (t)" ) +
  theme( legend.justification=c(0, 0), legend.position=c(0.02, 0.02),
    legend.background=element_rect(fill=alpha("white", 0.75), colour="black",
                                   size=0.2),
  ...
  ggsave( filename=file.path("knitr-figs", "MapSection.png"),
          width=figWidth/nMaps, height=figWidth/nMaps*xyRatioSec, dpi=600 )
# Plot Statistical Area
mapSA <- BaseMap +
  geom_path( data=filter(shapes$secDF, id%in%sectionsSA), aes(group=Section),
    size=0.25, colour="black" ) +
  geom_path( data=filter(shapes$secDF, id==zoomSec), aes(group=Section),
    size=0.75, colour="black" ) +
  geom_label( data=filter(shapes$secCentDF, Section%in%sectionsSA),
              alpha=0.75, aes(label=paste("Sec", Section, sep=" ")),
              size=3 ) +
  coord_cartesian( xlim=c(extSA$xmin, extSA$xmax),
    ylim=c(extSA$ymin, extSA$ymax), default=TRUE ) +
  theme( panel.border=element_rect(colour="black", fill=NA, size=0.5),
    aspect.ratio=xyRatioSA ) +
  ggsave( filename=file.path("knitr-figs", "MapStatArea.png"),
          width=figWidth/nMaps, height=figWidth/nMaps*xyRatioSA, dpi=600 )
# Plot SAR
mapReg <- BaseMap +
  geom_path( data=shapes$saDF, aes(group=StatArea), size=0.25,
    colour="black" ) +
  geom_path( data=filter(shapes$saDF, id==zoomSA), aes(group=StatArea),
             size=0.75, colour="black" ) +
  geom_label_repel( data=shapes$saCentDF, alpha=0.75,
    aes(label=paste("SA", StatArea, sep=" ")), size=3 ) +
  theme( panel.border=element_rect(colour="black", fill=NA, size=0.5) ) +
  ggsave( filename=file.path("knitr-figs", "MapRegion.png"),
          width=figWidth/nMaps, height=figWidth/nMaps/shapes$xyRatio, dpi=600 )
@
...
mapBC <- ggplot( data=shapes$landAllCropDF, aes(x=Eastings, y=Northings) ) +
  ...
  geom_path( data=shapes$regAllDF, aes(group=Region), size=0.5,
    colour="black" ) +
  geom_label( data=shapes$regCentDF, alpha=0.5, aes(label=Region), size=3 ) +
  annotate( geom="text", x=1050000, y=900000,
    label="British\nColumbia,\nCanada", size=4 ) +
  annotate( geom="text", x=700000, y=500000,
            label="Northeast\nPacific\nOcean", size=4 ) +
  coord_equal( ) +
...
We calculate the spawn index in four steps.
First, we develop a sampling protocol to estimate the number of eggs in a given area (\autoref{secStat}, \autoref{secSamp}).
Second, we develop a conversion factor to convert \fishName{} eggs to biomass (\autoref{secProd}), which is critical to calculating the spawn index.
Third, we calculate the spawn index for each of the three aforementioned spawn survey types: surface (\autoref{secSurf}), Macrocystis (\autoref{secMacro}), and understory (\autoref{secUnder}).
We separate calculations for each type in separate sections;
within each section, we separate each level of spatial aggregation (e.g., calculations at the quadrat, or transect level) into subsections (\autoref{figFlow}).
Finally, we combine the three spawn indices, and aggregate by stock assessment region and year to produce a relative index of combined sex spawning biomass (\autoref{secTotal}).

\begin{figure}
\centering
\begin{tikzpicture}[node distance=0.95cm and 0.1cm, font=\footnotesize]
\node [terminal, label={[text width=9.5em] \textbf{Macrocystis (M)}}] (mExt) {\labelMacro{mExt} Determine spatial extent and transect locations.};
\node [process, below=of mExt] (mPlant) {\labelMacro{mPlant} Plant $p$ observations for mature plants (e.g., number of stalks).};
\node [process, below=of mPlant] (mTrans) {\labelMacro{mTrans} Transect $t$ observations and calculations (e.g., spawn width, mean number of egg layers, mean plant height).};
\node [process, below=of mTrans] (mSpawn) {\labelMacro{mSpawn} Spawn $s$ observations and calculations (e.g., Macrocystis bed length, mean spawn width, mean egg density).};
\node [inout, below=of mSpawn] (mInd) {\labelMacro{mInd} Macrocystis spawn index $B_{xs}$.};
\node [terminal, label={[text width=9.5em] \textbf{Surface (S)}}, left=of mExt] (sExt) {\labelSurf{sExt} Determine spatial extent and sample locations (along transects if possible).};
\node [process, left=of mPlant] (sSamp) {\labelSurf{sSamp} Sample $j$ observations and calculations (e.g., number of egg layers, substrate type $i$).};
\node [process, below=of sSamp, left=of mSpawn] (sSpawn) {\labelSurf{sSpawn} Spawn $s$ observations and calculations (e.g., spawn length, spawn width, mean egg density).};
\node [inout, below=of sSpawn, left=of mInd] (sInd) {\labelSurf{sInd} Surface spawn index $B_{xs}$.};
\node [terminal, label={[text width=9.5em] \textbf{Understory (U)}}, right=of mExt] (uExt) {\labelUnder{uExt} Determine spatial extent, transect locations, and quadrat spacing.};
\node [process, below=of uExt, right=of mPlant] (uQuad) {\labelUnder{uQuad} Quadrat $q$ observations and calculations (e.g., number of egg layers, algae type $v$).};
\node [process, below=of uQuad, right=of mTrans] (uTrans) {\labelUnder{uTrans} Transect $t$ observations and calculations weighted by spawn width (e.g., spawn width, mean egg density).};
\node [process, below=of uTrans, right=of mSpawn] (uSpawn) {\labelUnder{uSpawn} Spawn $s$ observations and calculations (e.g., algae bed length, mean spawn width, egg density).};
\node [inout, below=of uSpawn, right=of mInd] (uInd) {\labelUnder{uInd} Understory spawn index $B_{xs}$.};
\node [inout, below=of mInd] (tIndex) {\labelTotal{tIndex} Total spawn index $B_s=\sum_{x=1}^X B_{xs}$.};
...
{Sequence of steps (e.g., \ref{sExt}, \ref{sSamp}) for \fishName{} spawn index calculations for the three spawn survey types $x = \{\text{surface}, \text{Macrocystis}, \text{understory}\}$.
Legend: rounded rectangles indicate start, rectangles indicate observations and calculations, parallelograms indicate output, and arrows show order of operation.}
\label{figFlow}
\end{figure}

We developed this document while converting spawn index calculations implemented in a \textbf{Microsoft Access} database to an \textbf{R} \citeyearpar[RCT][]{R-3.4.1} package, \texttt{SpawnIndex}.
...
For example, run the \textbf{R} package functions to implement the calculations described in this document using a small set of actual observations.
...
In contrast, underwater dive surveys using SCUBA gear instituted in 1988 follow a two-stage systematic sampling design where
...
First, mean egg density in eggs per square metre (m) for transect $t$ in spawn $s$ is
\begin{equation}\label{eqStatRhoTS}
\mean{\rho_{ts}} = \frac{1}{Q}\sum_{q=1}^Q \rho_{qts}
\end{equation}
where $\rho_{qts}$ is egg density per square metre (m) for quadrat $q$, and
$Q$ is the number of quadrats in transect $t$ (\autoref{tabNotationStat}).
...
$Q^\prime$ is the number of potential quadrats in transect $t$, and
...
$f^t$ & Transect sampling fraction & $(0,1]$ \\
$f^q$ & Quadrat sampling fraction & $(0,1]$ \\
...
to estimate mean egg density with a standard error of no more than 25\%.
The results indicated that the sampling required to achieve this level of precision included
surveying 3 transects per kilometre of spawn length, and
sampling at least 5 quadrats per transect (i.e., spawn width).
...
the protocol was found to be robust and has been in routine use since 1988 \citep{SchweigertEtal1990}.
...
An analogous approach had previously been adopted in the sampling of various commercial fisheries where vessels arrive in port at random
but are sampled in a systematic fashion to obtain a random sample \citep{QuinnEtal1983, Sen1984}.
...
After sampling dozens of giant kelp plants covered with herring eggs, it was determined that
plant height,
number of fronds per plant, and
number egg layers per plant
were key counts required to estimate the number of eggs per plant \citep{HaegeleSchweigert1990}.
The survey design employed to capture these data for each spawning bed rely on determining the
average plant height,
number of fronds in each plant holdfast, and
number of giant kelp plants occurring within a 1~m swath on each side of the transect line.
These data are used to determine the total egg deposition on \emph{Macrocystis} sp. for each spawning bed (\autoref{secMacroS}).
...
(i.e., $x = \{\text{surface}, \text{Macrocystis}, \text{understory}\}$),
...
for surface spawn surveys that don't use transects (\autoref{secSurfSamp}), surveyors collect longitude and latitude at the start and ends of the spawn (i.e., overall length and width).
...
\fishName{} spawn surveyors first determine the spatial extent of the spawn in terms of length of shoreline to survey (\autoref{figFlow}, steps \ref{sExt}, \ref{mExt}, \& \ref{uExt});
this is done by raking (\autoref{secSurfSamp}) or brief dives to determine the presence or absence of spawn.
...
In some cases, we adjust spawn width to improve the accuracy of spawn index estimates (\hyperref[secWidthAdjust]{appendix \ref{secWidthAdjust}}).

After determining the spatial extent, surveyors determine the number of egg layers on substrate and algae according to sampling protocols described in \autoref{secSurfSamp}, \autoref{secMacroSamp}, \& \autoref{secUnderSamp}.
...
\subsection{SURFACE SPAWN PROTOCOL}\label{secSurfSamp}
...
all surveys prior to 1988, and
surveys since 1988 when dive surveys are not possible.
...
From 1928 to 1978, surface spawn surveyors categorized spawn by subjective `intensity' categories instead of directly estimating the number of egg layers (\autoref{tab:Intensity}).
From 1928 to 1968 there were five intensity categories described as very light, light, medium, heavy, and very heavy (numbered 1 to 5, respectively).
Starting in 1969 there were nine intensity categories;
...
Starting in 1979, spawn surveyors estimated the number of egg layers directly, and they continued to record intensity categories until 1981 to provide overlap between the two methods.
In addition to the number of egg layers, intensity was sometimes recorded after being officially discontinued in 1981.
We have converted spawn intensity observations in the \fishName{} spawn survey database from five to nine categories for spawns that used the five category scale between 1951 and 1968.
...
<<Intensity>>=
# Caption
tabCap <- "Spawn intensity categories, description, and number of egg layers for \\fishName{} surface spawn surveys for the periods 1928 to 1968, and 1969 to 1978 \\citep{HayKronlund1987, SchweigertStocker1988}."
# Short caption
tabSCap <- "Spawn intensity categories and number of egg layers for \\fishName{} surface spawn surveys"
# Format the table
kIntensity <- intensity %>%
  arrange( Layers ) %>%
  rename( `1928 to 1968`=f1928t1968, `1969 to 1978`=f1969t1978,
    `Number of egg layers`=Layers) %>%
  kable( caption=tabCap, caption.short=tabSCap, booktabs=TRUE, linesep="",
    escape=FALSE ) %>%
  add_header_above( header=c("Intensity category"=2, "", "") )
# Print the table
kIntensity
@

\subsection{MACROCYSTIS SPAWN PROTOCOL}\label{secMacroSamp}
...
\subsection{UNDERSTORY SPAWN PROTOCOL}\label{secUnderSamp}
...
Female \fishName{} produce an average of approximately $200\,000$ eggs per kilogram (kg) of total body weight \citep{Hay1985, HayBrett1988}.
...
\end{equation}
where $\omega$ is the number of eggs per kilogram of total female body weight,
$\phi^f$ is the proportion of spawners that are female, and
$\theta$ is the egg conversion factor (\autoref{tabNotationEggs}).
Thus we convert eggs to biomass in tonnes of both sexes combined by dividing the number of eggs by $\theta$.
Although \fishName{} egg production is affected by environmental variability and other factors \citep{TanasichukWare1987, HayBrett1988}, we assume that bias to the spawn index from \autoref{eqECF} is insignificant in most areas and years \citep{Schweigert1993b, Ware1985}.

\begin{table}
\centering
\caption[Notation for converting \fishName{} eggs to biomass]
{Notation for converting the number of \fishName{} eggs to biomass.
Legend: kilograms (kg), tonnes (t).}
\begin{tabulary}{\linewidth}{lLLL}
\toprule
Name & Description & Value or unit & Reference \\
\midrule
$\omega$ & Female fecundity & $200\,000~\text{eggs} \cdot \text{kg}^{-1}$ & \cite{Hay1985, HayBrett1988} \\
$\phi^f$ & Proportion female & 0.5 & \cite{Hay1985, HayBrett1988} \\
...
Surface spawn surveyors sample along transects or using their judgement.
Surveyors collect data for at the substrate type $i$, sample $j$, and spawn $s$ levels;
we calculate metrics at the substrate type $i$, sample $j$, and spawn $s$ levels (\autoref{tabNotationSurf}).
Recall that surface spawn `samples' include observations collected using specialized rakes and viewing boxes (\autoref{secSurfSamp}).
Occasionally, we use field data sheets to fill-in missing egg layer information for surface survey data (\hyperref[secUpdate]{appendix \ref{secUpdate}}).
...
Name & Description & Value or unit & Reference \\
\midrule
$E^\prime$ & Number of egg layers & $> 0$ & \\
$\phi^b$ & Proportion of substrate covered in eggs & $(0,1]$ & \\
$E$ & Number of egg layers & $> 0$ & \\
$\alpha$ & Regression intercept & $14.698~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{SchweigertEtal1997} \\
$\beta$ & Regression slope & $212.218~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{SchweigertEtal1997} \\
...
\subsection{SAMPLE OBSERVATIONS AND CALCULATIONS}
...
\citet{SchweigertEtal1997} developed a predictive model of surface egg density as a function of number of egg layers using a linear regression model%
...
EggDensSurf <- function( EggLyrs ) {
  ...
  Density <- EggLyrs * 212.218 + 14.698
  # Return density
  return( Density )
...
# Figure
EggDensSubPlot <- ggplot( data=dat,
  mapping=aes(x=Layers, y=EggDens) ) +
  geom_line( ) +
  # geom_point( ) +
  labs( x="Number of egg layers",
    y=expression(paste("Egg density (eggs"%.%10^3%.%"m"^-2,")", sep="")) ) +
...
\subsection{SPAWN OBSERVATIONS AND CALCULATIONS}
...
Macrocystis spawn surveyors use SCUBA gear to collect underwater data for individual plants $p$, transects $t$, and spawns $s$;
...
Name & Description & Value or unit & Reference \\
\midrule
$K$ & Number of stalks & $> 0$ & \\
$W$ & Spawn width & m & \\
$\chi$ & Transect swath & m & \\
$A$ & Area & $\text{m}^2$ & \\
$\mean{H}$ & Mean plant height & m & \\
$\mean{E}$ & Mean number of egg layers & $> 0$ & \\
$L^m$ & Length of the Macrocystis bed & m & \\
$L$ & Spawn length & m & \\
$A$ & Area of spawn $s$ & $\text{m}^2$ & \\
$P$ & Number of plants & $> 0$ & \\
$\mean{\kappa}$ & Mean number of stalks per plant & $> 0$ & \\
$\beta$ & Regression slope & $0.073~\text{eggs} \cdot 10^3 \cdot \text{plant}^{-1}$ & \cite{HaegeleSchweigert1990} \\
$\gamma$ & Regression exponent on $\mean{E}$ & 0.673 & \cite{HaegeleSchweigert1990} \\
$\delta$ & Regression exponent on $\mean{H}$ & 0.932 & \cite{HaegeleSchweigert1990} \\
$\epsilon$ & Regression exponent on $\mean{\kappa}$ & 0.703 & \cite{HaegeleSchweigert1990} \\
...
\subsection{TRANSECT OBSERVATIONS AND CALCULATIONS}
...
\end{equation}
where $W_{ts}$ is the transect width,
...
The total number of stalks in spawn $s$ is
...
The mean number of egg layers in spawn $s$ is
...
The mean number of stalks per plant in spawn $s$ is
...
\citet{HaegeleSchweigert1990} developed a predictive model of number of eggs per plant as a function of number of egg layers on plants, plant height, and number of stalks per plant using a nonlinear multiple regression model
\begin{equation}\label{eqMacroEggsPerPlantS}
\mean{\psi_s} = \beta \mean{E_s}^{\gamma} \mean{H_s}^{\delta} \mean{\kappa_s}^{\epsilon} 10^3
\end{equation}
where $\beta$ is the regression slope,
...
NumEggs <- function( EggLyrs, Height, StalksPerPlant ) {
  ...
  Number <- 0.073 * EggLyrs^0.673 * Height^0.932 * StalksPerPlant^0.703 * 1000
  # Return number
  return( Number )
}  # End NumEggs function
# Make a table of egg layers, proportion, coefficient, and egg density
dat <- as_tibble( df ) %>%
  mutate( Number=NumEggs(EggLyrs=Layers, Height=PlantHeight,
    StalksPerPlant=Stalks),
    Stalks=paste("Number of stalks per plant: ",
      formatC(Stalks, width=2, flag="0"), sep="") )
# Figure
NumEggPlot <- ggplot( data=dat,
  mapping=aes(x=Layers, y=Number, group=PlantHeight) ) +
  geom_line( mapping=aes(colour=PlantHeight) ) +
  # geom_point( ) +
  labs( x="Number of egg layers on plants",
    y=expression(paste("Number of eggs per plant (eggs"%.%10^3%.%"plant"^-1,
      ")", sep="")),
...
Understory spawn surveyors use SCUBA gear to collect underwater data for algae (i.e., vegetation) types $v$, quadrats $q$, transects $t$, and spawns $s$;
...
Name & Description & Value or unit & Reference \\
\midrule
$E^b$ & Number of substrate egg layers & $> 0$ & \\
$\phi^b$ & Proportion of substrate covered in eggs & $(0,1]$ & \\
$\alpha$ & Regression slope for substrate & $340~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{HaegeleEtal1979} \\
$\rho^b$ & Substrate egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$\beta$ & Regression slope for algae & $600.567~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{Schweigert2005} \\
$E^v$ & Number of algae egg layers & $> 0$ & \\
$\gamma$ & Regression exponent on $E^v$ & 0.6355 & \cite{Schweigert2005} \\
$\phi^v$ & Proportion of algae covered in eggs & $(0,1]$ & \\
$\delta$ & Regression exponent on $\phi^v$ & 1.4130 & \cite{Schweigert2005} \\
$C$ & Algae coefficient & see \autoref{tab:AlgaeCoefs} & \\
$\rho^v$ & Algae egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$\rho$ & Egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$W$ & Spawn width & m & \\
$\mean{\rho^\prime}$ & Mean linear weighted egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-1}$ & \\
$\mean{W}$ & Mean spawn width & m & \\
$L^v$ & Length of the algae bed & m & \\
$L$ & Spawn length & m & \\
$\rho^\prime$ & Sum of linear weighted egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-1}$ & \\
...
\subsection{QUADRAT OBSERVATIONS AND CALCULATIONS}
...
\citet{HaegeleEtal1979} developed a predictive model of substrate egg density in quadrat $q$, transect $t$, and spawn $s$ from egg layers using a linear regression model
\begin{equation}\label{eqUnderDensSubQTS}
\rho^b_{qts} = \alpha E^b_{qts} \phi^b_{qts}
\end{equation}
where $\alpha$ is the regression slope,
...
# Figure
EggDensSubPlot <- ggplot( data=dat,
  mapping=aes(x=Layers, y=EggDens, group=Proportion) ) +
  geom_line( mapping=aes(colour=Proportion) ) +
  labs( x="Number of egg layers on substrate",
...
Although quadrats have only one substrate, they can have one or more algae types $v$.
\citet{Schweigert2005} developed a predictive model of algae egg density from egg layers, proportion of the quadrat covered by algae, and an algae coefficient using a generalized linear model.
...
\begin{equation}\label{eqUnderDensAlgAQTS}
\rho^v_{aqts} = \beta E^{v^\gamma}_{aqts} \phi^{v^\delta}_{aqts} C_v
\end{equation}
where $\beta$ is the regression slope,
$E^v_{aqts}$ is the number of egg layers on algae $v$,
$\gamma$ is the regression exponent on $E^v_{aqts}$,
$\phi^v_{aqts}$ is the proportion of quadrat $q$ covered by algae $v$,
$\delta$ is the regression exponent on $\phi^v_{aqts}$,
$C_v$ is the coefficient for algae $v$, and
$\rho^v_{aqts}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ (\autoref{figUnderDensAlgAQTS}).
The total algae egg density for quadrat $q$ in transect $t$ and spawn $s$ is
\begin{equation}\label{eqUnderDensAlgQTS}
\rho^v_{qts} = \sum_{a=1}^{A} \rho^v_{aqts}
\end{equation}
where $\rho^v_{aqts}$ is egg density in algae $v$ from \autoref{eqUnderDensAlgAQTS}, and
$\rho^v_{qts}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$.

<<AlgaeCoefs>>=
# Caption
tabCap <- "Algae (i.e., vegetation) types $v$ and coefficients $C$ for \\fishName{} understory spawn surveys \\citep{Schweigert2005}."
# Short caption
tabSCap <- "Algae types and coefficients for \\fishName{} understory spawn surveys"
# Format the table
kAlgaeCoefs <- algaeCoefs %>%
  rename( `Algae type $v$`=AlgaeType, `Coefficient $C$`=Coefficient ) %>%
  kable( caption=tabCap, caption.short=tabSCap, booktabs=TRUE, linesep="",
    escape=FALSE )
# Print the table
kAlgaeCoefs
...
<<UnderDensAlgAQTS, fig.width=6, fig.asp=1, fig.lp="fig", echo=FALSE, fig.cap=figCap, fig.scap="Algae egg density for \\fishName{} underwater spawn surveys">>=
# Caption
figCap <- paste("Algae egg density in thousands of eggs per square metre (m) as a function of number of egg layers on algae, proportion of substrate covered in algae, and algae coefficient (\\autoref{tab:AlgaeCoefs}) for \\fishName{} underwater spawn surveys (\\autoref{eqUnderDensAlgAQTS}; \\citealt{Schweigert2005}). Note that number of egg layers can exceed those shown in this figure; values shown are for demonstration only.")
# Expand the values
df <- expand.grid( Layers=lyrs, Proportion=prop,
  Coefficient= unique( algaeCoefs$Coefficient ) )
# Equation for understory egg density on algae
EggDensAlg <- function( AlgLyrs, AlgProp, Coef ) {
  # Calculate egg density
  Density <- 600.567 * AlgLyrs^0.6355 * AlgProp^1.4130 * Coef
  # Return density
  return( Density )
...
dat <- as_tibble( df ) %>%
  arrange( Coefficient ) %>%
  mutate( EggDens=EggDensAlg(AlgLyrs=Layers, AlgProp=Proportion,
    Coef=Coefficient),
    Coefficient=paste("Algae coefficient: ", format(Coefficient, digits=4),
      sep="") )
# Figure
EggDensAlgPlot <- ggplot( data=dat,
  mapping=aes(x=Layers, y=EggDens, group=Proportion) ) +
  geom_line( mapping=aes(colour=Proportion) ) +
  labs( x="Number of egg layers on algae",
    y=expression(paste("Algae egg density (eggs"%.%10^3%.%"m"^-2,")", sep="")),
    colour="Proportion of substrate\ncovered in algae") +
  scale_y_continuous( labels=comma ) +
  scale_colour_viridis( ) +
...
% TODO: Is the linear weighted egg density necessary? Seems complicated and maybe a simpler way is possible?
At the transect level (\autoref{figFlow}, step \ref{uTrans}), the mean linear understory egg density in transect $t$ and spawn $s$ is
\begin{equation}\label{eqUnderDensLinTS}
\mean{\rho^\prime_{ts}} = W_{ts} \frac{1}{Q} \sum_{q=1}^Q \rho_{qts}
\end{equation}
where $W_{ts}$ is the spawn width in metres,
$Q$ is the number of quadrats in transect $t$,
$\rho_{qts}$ is total understory egg density in quadrat $q$ from \autoref{eqUnderDensQTS}, and
$\mean{\rho^\prime_{ts}}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-1}$.
We calculate a weighted mean egg density because spawn width can vary along the spawn length;
a weighted mean ensures that transects contribute proportionally to their area.
Note that we update spawn width to correct for errors regarding the assumed accuracy of transect lines used to measure spawn width for understory surveys between 2003 and 2014 (\autoref{secWidthUnder}).

\subsection{SPAWN OBSERVATIONS AND CALCULATIONS}

At the spawn level (\autoref{figFlow}, step \ref{uSpawn}), the sum of spawn widths in spawn $s$ is
\begin{equation}\label{eqUnderWidthSumS}
W_s = \sum_{t=1}^T W_{ts}
\end{equation}
where $W_{ts}$ is the spawn width in transect $t$, and
$W_s$ is in metres.
The mean width of spawn $s$ is
\begin{equation}\label{eqUnderWidthMuS}
\mean{W_s} = \frac{W_s} {T}
\end{equation}
where $W_s$ is the sum of widths from \autoref{eqUnderWidthSumS},
$T$ is the number of transects in spawn $s$, and
$\mean{W_s}$ is in metres.
...
Thus, we assume that eggs on the substrate and eggs on algae are represented by the same length measurement.
The sum of transect egg densities in spawn $s$ is
\begin{equation}\label{eqUnderDensLinS}
\rho^\prime_s = \sum_{t=1}^T \mean{\rho^\prime_{ts}}
\end{equation}
where $\mean{\rho^\prime_{ts}}$ is the mean linear weighted understory egg density in transect $t$ from \autoref{eqUnderDensLinTS}, and
$\rho^\prime_s$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-1}$.
Next we convert from linear density to area density.
Understory egg density in spawn $s$ is
\begin{equation}\label{eqUnderDensS}
\rho_s = \frac{\rho^\prime_s} {W_s}
\end{equation}
where $\rho^\prime_s$ is the sum of transect egg densities in spawn $s$ from \autoref{eqUnderDensLinS},
$W_s$ is the sum of spawn widths in spawn $s$ from \autoref{eqUnderWidthSumS}, and
$\rho_s$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$.
The understory spawn index in spawn $s$ is
\begin{equation}\label{eqUnderIndS}
B_s = \frac{\rho_s L^v_s \mean{W_s} 10^3} {\theta}
\end{equation}
where $\rho_s$ is understory egg density from \autoref{eqUnderDensS},
...
$B_{snry}$ is the total spawn index in tonnes (\autoref{figFlow}, step \ref{tIndex}).
...
Spawn on kelp (SOK) fisheries collect \fishName{} roe that adhere to algae such as Macrocystis after spawning.
...
The most direct is to estimate the quantity of eggs removed from the population, and treat them as though they would have spawned and contributed to spawning biomass.
...
Nevertheless, \cite{WhyteEnglar1977} determined that following a 24\,hour brining period, wet product weight increases by about 13\% due to salt uptake.
...
$\nu$ is the proportion of SOK product that is eggs, not kelp,
$\upsilon$ is the proportion of SOK product that is eggs after brining,
$M$ is the average mass in kilograms of a fertilized egg, and
...
$\nu$ & Proportion of SOK product that is eggs, not kelp & 0.88 & \cite{ShieldsEtal1985} \\
$\upsilon$ & Proportion of SOK product that is eggs after brining & $\frac{1}{1.13}$ & \cite{WhyteEnglar1977} \\
$M$ & Average mass of a fertilized egg & $2.38 \cdot 10^{-6}\,\text{kg}$ & \cite{HayMiller1982} \\
...
Some examples illustrate these sources of uncertainty: % (\autoref{tabUncertainty}).
\begin{enumerate}
\item Natural variability could affect \fishName{} fecundity, and the sex ratio of spawning \fishName{} (\autoref{eqECF}).
Fecundity could be influenced by biological processes such as the observed non-stationarity of weight-at-age, or a truncated age distribution caused by selective fishing or high natural mortality.
\item Measurement error could affect input data such as the number of egg layers, while model structural complexity could affect estimated prediction model parameters, or the form of their relationship, or both (e.g., \autoref{eqSurfDensJS}).
In addition, spawn index prediction models are dated, and our understanding of these processes could have changed in the intervening years.
...
\item Uncertainty in the number of egg layers for spawn intensity catetories (\autoref{tab:Intensity}), and algae coefficients (\autoref{tab:AlgaeCoefs}).
Again, uncertainty in these values is currently unknown.
\end{enumerate}
Despite these assumptions and potential sources of uncertainty, the spawn index has typically been reported without quantifying uncertainty \citep[but see][]{SchweigertEtal1993}.
...
\begin{enumerate}
\item Investigate factors that influence fecundity and sex ratios (\autoref{eqECF});
...
Sampling surveys trade off precision of estimated quantities versus survey effort or cost.
...
\item Quantify the precision and accuracy of spawn width estimates (e.g., \hyperref[secWidthAdjust]{appendix \ref{secWidthAdjust}});
\item Review transect and quadrat spacing \citep[\autoref{secStat}; see][]{Schweigert1993b};
\item Review egg prediction model accuracy (\autoref{eqECF});
\item Review temporal stability of egg layer estimates; and
...
Many of the parameters and prediction models used to calculate the spawn index are dated;
these analyses could be checked with new information, and updated if required.
Parameters include $\omega$, $\phi^f$, $\nu$, $\upsilon$, and $M$.
Prediction models include \autoref{eqSurfDensJS}, \autoref{eqMacroEggsPerPlantS}, \autoref{eqUnderDensSubQTS}, and \autoref{eqUnderDensAlgAQTS}.
In addition, parameter and prediction model uncertainty should be propagated through the calculations to quantify spawn index uncertainty (\autoref{secUncertain}).
One approach to account for prediction model uncertainty is to incorporate the underlying data that informs these equations into spawn index calculations.
In addition, future work could review the assumed statistical framework.

\section{DOWNLOAD}\label{secDown}

The \textbf{R} package to calculate the \fishName{} spawn index, \texttt{SpawnIndex} is publicly accessible on the \href{https://github.com/grinnellm/SpawnIndex}{\fishName{} spawn index repository}.
The package includes an example database of \fishName{} spawn survey observations.
The \textbf{R} package contains the functions to imports tables from the database, and calculate the spawn index.
This document is meant to accompany the \textbf{R} package, which implements the calculations described here.
The \textbf{R} package is documented to promote accessibility and transparency.
...
There are two cases where we adjust spawn width estimates to improve spawn index accuracy: surface surveys in all years from 1951 to present, and understory dive surveys between 2003 and 2014.

\subsection{SURFACE SPAWN WIDTH}\label{secWidthSurface}

Surface surveys were the only survey type prior to 1988, while the majority of spawns since 1988 have been surveyed using SCUBA gear.
...
To improve the consistency of spawn index estimates throughout the time period from 1951 to present, we adjust surface spawn width estimates using underwater estimates when available \citep{SchweigertEtal1993}.
...
We summarise spawn width by the median instead of the mean because widths are not normally distributed \citep{SchweigertEtal1993}.
If there are no dive data that meet those criteria, we use the median width from all dive surveys within the Section, or within the SAR if there are no dives within the Section.
If there are still no dive data that meet those criteria, we use the observed width $W^\prime$ from the surface survey.
...
these lines are marked in $1\,\text{m}$ increments, and are standardized to $20\,\text{m}$ segments.
Segments refer to individual sections of line, which may be linked together to make complete transects.

Sometime in the mid- to late-1990s, spawn surveyors observed that the $20\,\text{m}$ segments shrank by approximately $1\,\text{m}$ during the first season of use, and continued to shrink over time.
DFO staff noticed that this issue was occurring coastwide, and began re-measuring lead lines each season.
...
(i.e., inner segments are replaced more frequently than seaward segments, and
...
The earliest written instructions that describe the modified protocol of marking $1.15\,\text{m}$ increments is from 2003, and this protocol was used until 2013.
Note that some SARs continued to use old lead lines in 2014.
...
Given the observations summarized above, we adjust spawn width estimates based on written instructions for the marking protocol in 2003.
Accordingly, our best estimate of years impacted by marking lead lines at $1.15\,\text{m}$ increments (when shrinking no longer occurred) is from 2003 to 2014.
...
\end{equation}
where $W^\prime_{txsnry}$ is the observed spawn width for transect $t$ in spawn survey type $x$, spawn $s$, Location $n$, SAR $r$, and year $y$,
...
Instead of updating the database permanently, we adjust spawn widths in the \textbf{R} package script to be transparent, and to prevent a mismatch between the original data sheets and the database.
...
# Format the table
kWidthFac <- adjWidthFacs %>%
  mutate_if( is.double, funs(format(., nsmall=3)) ) %>%
  mutate( Year=as.integer(Year) ) %>%
  arrange( Year ) %>%
  kable( caption=tabCap, caption.short=tabSCap, booktabs=TRUE, linesep="",
    escape=FALSE, digits=c(0, 3, 3, 3, 3, 3, 3, 3) ) %>%
  add_header_above( header=c("", "SAR"=7) )
# Print the table
kWidthFac
@
...
One record in the surface spawn database since 1951 requires an update to fill-in missing egg layer information.
As with understory spawn width updates, we make this update in the \textbf{R} package script.
This affects the following record:
\begin{enumerate}
\item Update `intensity' from 0 to 1 for the 1 record in the year 1962, Statistical Area 14, Section 142, Location code 820, and with $\text{intensity} = 0$.
We update intensity from 0 to 1 because spawn was surveyed but not reported.
\end{enumerate}
Spawn survey records prior to 1951 have additional missing or inaccurate egg layer information, and are unreliable for indexing purposes.
Therefore, we do not include spawn data prior to 1951 in stock assessments.

While reviewing the spawn index calculations and translating them from the \textbf{Microsoft Access} database to \textbf{R}, we found several cases where good quality spawn index data were being over-written with no documented reason.
These updates have been omitted, and affected the following records:
\begin{enumerate}
\item Update $E_{js}$ to 2.1496 for the 15 records in the year 1979, Statistical Area 2, and with intensity 4;
\item Update $E_{js}$ to 0.5529 for the 4 records in the year 1981, Statistical Area 24, and with $E_{js} = 0.0$;
\item Update $E_{js}$ to 1.3360 for the 7 records in the year 1982, Statistical Area 23, and with intensity 3;
\item Update $E_{js}$ to 2.3300 for 41 records in the year 1984, Statistical Area 24, and with intensity 0; and
\item Update $E_{js}$ to 2.9800 for 14 records in the year 1982, Statistical Area 27, and with $E_{js} = 0.0$.
\end{enumerate}
...
These changes had negligible effects on spawn index values.
