...
BaseMap <- ggplot(data = shapes$landCropDF, aes(x = Eastings, y = Northings)) +
  geom_polygon(
    data = shapes$landCropDF, aes(group = group),
    fill = "darkgrey"
  ) +
...
mapBC <- ggplot(data = shapes$landAllCropDF, aes(x = Eastings, y = Northings)) +
  ...
  geom_path(
    data = shapes$regAllDF, aes(group = Region), size = 0.5, olour = "black"
  ) +
  geom_label(
...
<<Intensity>>=
# Caption
tabCap <- paste("Spawn intensity categories, description, and number of egg layers for \\fishName{} surface spawn surveys for the periods 1928 to ", cat9Yr-1, ", and ", cat9Yr, " to ", directEggYr, " \\citep{HayKronlund1987, SchweigertStocker1988}.", sep="")
# Short caption
...
kIntensity <- intensity %>%
  arrange(Layers) %>%
  rename(
    `1928 to 1968` = Period5,
    `1969 to 1978` = Period9,
...
$\phi^f$ & Proportion female & \Sexpr{pars$conversion$phi} & \cite{Hay1985, HayBrett1988} \\
...
$\beta$ & Regression slope & \Sexpr{pars$macrocystis$beta}$~\text{eggs} \cdot 10^3 \cdot \text{plant}^{-1}$ & \cite{HaegeleSchweigert1990} \\
...
NumEggs <- function(EggLyrs, Height, StalksPerPlant) {
  ...
  Number <- pars$macrocystis$beta * EggLyrs^pars$macrocystis$gamma *
...
$\phi^b$ & Proportion of substrate covered in eggs & $(0,1]$ & \\
$\alpha$ & Regression slope for substrate & \Sexpr{pars$understory$alpha}$~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{HaegeleEtal1979} \\
$\rho^b$ & Substrate egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$\beta$ & Regression slope for algae & \Sexpr{pars$understory$beta}$~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{Schweigert2005} \\
$E^v$ & Number of algae egg layers & $> 0$ & \\
$\gamma$ & Regression exponent on $E^v$ & \Sexpr{pars$understory$gamma} & \cite{Schweigert2005} \\
$\phi^v$ & Proportion of algae covered in eggs & $(0,1]$ & \\
$\delta$ & Regression exponent on $\phi^v$ & \Sexpr{pars$understory$delta} & \cite{Schweigert2005} \\
...
EggDensSub <- function(SubLyrs, SubProp) {
  # Calculate egg density
  Density <- pars$understory$alpha * SubLyrs * SubProp
  # Return density
  return(Density)
...
# Expand the values
df <- expand.grid(
  Layers = lyrs, Proportion = prop,
  Coefficient = unique(algaeCoefs$Coef)
)
# Equation for understory egg density on algae
EggDensAlg <- function(AlgLyrs, AlgProp, Coef) {
  # Calculate egg density
  Density <- pars$understory$beta * AlgLyrs^pars$understory$gamma *
    AlgProp^pars$understory$delta * Coef
  # Return density
  return(Density)
...
$\nu$ & Proportion of SOK product that is eggs, not kelp & \Sexpr{pars$SOK$nu} & \cite{ShieldsEtal1985} \\
$\upsilon$ & Proportion of SOK product that is eggs after brining & \Sexpr{round(pars$SOK$upsilon, digits=4)} & \cite{WhyteEnglar1977} \\
...
In addition to the issues regarding uncertainty (\autoref{secUncertain}),
there are a few caveats to interpreting the \fishName{} spawn index, and using this data for analyses:
\begin{enumerate}
\item the spawn index a relative index of spawning biomass,
\item the spawn index is derived data with uncertainty and assumptions, as opposed to observed data,
\item there are two different survey periods with substantial differences in survey effort and method (surface surveys from \Sexpr{min(yrRange)} to \Sexpr{newSurvYr-1}, and dive surveys from \Sexpr{newSurvYr} to present),
\item the surface survey period used two different methods to estimate the number of egg layers:
spawn intensity categories (five categories from \Sexpr{min(yrRange)} to \Sexpr{cat9Yr-1}, and nine categories from \Sexpr{cat9Yr} to \Sexpr{directEggYr-1}), and
direct estimates (from \Sexpr{directEggYr} to \Sexpr{newSurvYr-1}), and
\item the spawn index is presence only and does not indicate the absence of spawn.
\end{enumerate}
