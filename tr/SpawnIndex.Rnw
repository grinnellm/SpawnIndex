...
We use $B'_{ry}$ as an indicator of \fishName{} relative population abundance (i.e., biomass) in stock assessment models \citeyearpar[DFO][]{DFO2020}.
...
not the quadrat number as in other sections of this report.
Briefly, $q$ accounts \fishName{} eggs lost to predation prior to being measured, and for unobserved spawns \citeyearpar[DFO][]{DFO2016}.
Because of differences between the two spawn survey periods, we estimate one spawn index scaling parameter for
