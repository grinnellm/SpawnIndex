...
\item There are two different survey periods with substantial differences in survey effort and method (surface period from \Sexpr{min(yrRange)} to \Sexpr{newSurvYr-1}, and dive period from \Sexpr{newSurvYr} to present),
