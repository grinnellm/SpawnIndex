...
To facilitate integrating this document with the \textbf{R} package,
we provide tables to cross-reference parameters and equations/functions
(\autoref{seccCross}).
...
\section{CROSSWALK TABLES}\label{seccCross}
...
Two crosswalk tables facilitate integrating this document with the SpawnIndex \textbf{R} package.
First, we cross-reference equations in this document with
functions in the \textbf{R} package (\autoref{tabLookupFuns}).
Second, we cross-reference parameters between this document and
the \textbf{R} package (\autoref{tabLookupPars}).

\begin{table}
\centering
\caption[Crosswalk table for functions in the SpawnIndex \textbf{R} package]
{Crosswalk table for equations in this technical report and functions in the SpawnIndex \textbf{R} package (\gitRepo{}).}
\begin{tabulary}{\linewidth}{LLLl}
\toprule
...
\begin{table}
\centering
\caption[Crosswalk table for parameters in the SpawnIndex \textbf{R} package]
{Crosswalk table for parameters in this technical report and the SpawnIndex \textbf{R} package (\gitRepo{}).}
...
\midrule
Female fecundity & $\omega$ & \autoref{tabNotationEggs}, \autoref{eqECF} & \texttt{omega} & \texttt{pars}, \texttt{eggs\_to\_sb} \\
Proportion female & $\female$ & \autoref{tabNotationEggs}, \autoref{eqECF} & \texttt{female} & \texttt{pars}, \texttt{eggs\_to\_sb} \\
Egg conversion factor & $\theta$ & \autoref{tabNotationEggs}, \autoref{eqECF} & \texttt{theta} & \texttt{eggs\_to\_sb} \\
\bottomrule
\end{tabulary}
\label{tabLookupPars}
\end{table}
