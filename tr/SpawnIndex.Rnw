...
$k_{ptxsnry}$ & Stalk & 1, 2, 3, ..., $K_{ptxsnry}$ \\
...
We calculate the total number of stalks $k$ in transect $t$
\begin{equation}\label{eqMacroStalksTS}
K_{t_{xsnry}} = \sum_{p_{txsnry}=1}^{P_{tsxnry}} K_{p_{tsxnry}},
\end{equation}
...
the Macrocystis bed length is $L_{s_nry}'$ in metres.
If $L_{s_nry}'$ is inadvertently not recorded,
we set $L_{s_nry}'$ to the spawn length $L_{s_nry}$.
...
if $L_s''$ is inadvertently not recorded, we set $L_s''$ to the spawn length $L_s$.
...
We estimate the spawn index for \fishName{} that spawned and produced eggs which were removed from the population by SOK fishery $g$ (\autoref{tabNotationIndices}) as
