...
Macrocystis and understory spawn surveys are conducted using SCUBA gear, and have been used for all major spawns since \Sexpr{newSurvYr}.
...
From \Sexpr{firstYr} to \Sexpr{directEggYr-1}, surface spawn surveyors categorized spawn by subjective `intensity' categories instead of directly estimating the number of egg layers (\autoref{tab:Intensity}).
...
<<Intensity>>=
# Caption
tabCap <- paste("Spawn intensity categories, description, and number of egg layers for \\fishName{} surface spawn surveys for the periods", firstYr, "to ", cat9Yr - 1, ", and ", cat9Yr, " to ", directEggYr, " \\citep{HayKronlund1987, SchweigertStocker1988}.", sep = "")
# Short caption
...
There are two cases where we adjust spawn width estimates to improve spawn index accuracy: surface surveys in all years from \Sexpr{min(yrRange)} to present, and understory dive surveys between 2003 and 2014.
...
we adjust surface spawn width estimates using underwater estimates when available \citep{SchweigertEtal1993}.
