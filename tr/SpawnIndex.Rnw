...
Each type of spawn survey observation has a specific statistical framework:
surface observations, and
two types of dive observations:
...
\endfirsthead
\toprule
\multicolumn{3}{c}{\textsl{\tablename~\thetable~continued}} \\
\textbf{Name} & \textbf{Description} & \textbf{Range} \\
\midrule
\endhead
\bottomrule
...
\end{equation}
where $\rho_{qts}$ is egg density in thousands of eggs per square metre (m) for quadrat $q$ ($10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$),
