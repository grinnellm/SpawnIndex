...
get_spawn <- function(fn) {
  ...
  if (is_data) {
    ...
    dat <- read_csv(
      file = "https://pacgis01.dfo-mpo.gc.ca/FGPPublic/Pacific_Herring_Spawn_Index_Data/Pacific_herring_spawn_index_data_EN.csv",
      col_types = cols()
    ) %>%
...
Recall that spawn index calculations so far
...
are not necessarily nested and hierarchical (e.g., $B_{xsnry}'$);
this is the finest scale that we calculate the spawn index (\autoref{secProt}).
Thus, we use the notation of \cite{Montgomery2012}
...
We sum over survey type $x$ to calculate
