...
There are \Sexpr{num_to_word(nrow(regions))} SARs in BC,
...
The minor SARs are \Sexpr{paste_nicely(regions$RegionName[!regions$Major])}.
...
\end{equation}
where $Q_{txsnry}'$ is the number of potential quadrats in transect $t$
...
$\mean{Q_{xsnry}'}$ is the mean number of potential quadrats in spawn $s$ from \autoref{eqStatQSPrime},
$Q'_{ts}$ is the number of potential quadrats in transect $t$,
$\mean{\rho_{t_{xsnry}}}$ is the mean transect egg density from \autoref{eqStatRhoT}, and
$\mean{\rho_{s_{nry}}}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
...
$s_{xnry}=1,\,2,\,3,\,...,\,S_{xnry}$.
Spawn numbers identify distinct spawns,
which we define as a continuous stretch of shoreline with no detectable break in egg deposition;
...
E_{i_{jxsnry}} = E'_{i_{jxsnry}} \phi_{i_{jxsnry}},
\end{equation}
where $E'_i$ is the number of egg layers on substrate $i$,
...
For the time period when surveyors recorded spawn `intensity' categories instead of direct egg layer estimates, we convert intensity to number of egg layers $E_j$ (\autoref{tab:Intensity}).
...
spawn length $L_{s_{nry}}$, and
spawn width $W_{s_{nry}}$, both in metres.
We set $W_{s_{nry}}$ to the first non-missing value of
median pool width, median Section width, median SAR width, or observed width $W_{s_{nry}}'$
...
B'_{s_{nry}} = \frac{\mean{\rho_{s_{nry}}} L_{s_{nry}} W_{s_{nry}} 10^3} {\theta},
\end{equation}
...
dat <- as_tibble(df) %>%
  mutate(EggDens = dens_under_sub(
    sub_layers = Layers, sub_prop = Proportion
  ))
# Figure
egg_dens_sub_plot <- ggplot(
...
dat <- as_tibble(df) %>%
  ...
  mutate(
    EggDens = dens_under_alg(
      alg_layers = Layers, alg_prop = Proportion, coef = Coefficient
    ),
...
Instead of updating the database permanently, we adjust spawn width in the \textbf{R} package to be transparent, and to prevent mismatches between the original data sheets and databases.
...
As with understory spawn width updates, we make this update in the \textbf{R} package:
\begin{enumerate}
...
\begin{enumerate}
\item Update $E_j$ (i.e., the total number of egg layers) to 2.1496 for the records (there are 15 records) in the year 1979, Statistical Area 2, and with intensity 4;
...
\end{enumerate}
In the first three cases, $E_j$ was updated using intensity categories (\autoref{tab:Intensity});
in the last two cases, $E_j$ was updated using historical averages.
