...
we provide tables to cross-reference parameters and equations
(\autoref{secCross}).
...
\section{CROSSWALK TABLES}\label{secCross}
...
Two crosswalk tables facilitate pairing this document with the SpawnIndex \textbf{R} package.
First, we cross-reference equations in this document with
