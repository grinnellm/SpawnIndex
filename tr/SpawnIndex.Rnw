...
\begin{enumerate}
\item The database has been used for various purposes over the last two decades
and has incidental calculations that make it overly complex,
\item The database is difficult to troubleshoot because
it is hard to differentiate between input (i.e., data) and derived values,
\item An \textbf{R} package is a step towards `good enough' practices in scientific computing \citep{WilsonEtal2016},
...
\item The \textbf{R} package clearly separates data from analyses,
\item An \textbf{R} package will facilitate future research to quantify spawn index uncertainty, and
\item An \textbf{R} package will allow us to generate dynamic documents
