...
there are a few caveats to keep in mind when interpreting the \fishName{} spawn index, and using the spawn index in analyses:
\begin{enumerate}
\item the spawn index a relative index of spawning biomass,
\item the spawn index is derived data with uncertainty and assumptions, as opposed to observed data (\autoref{secUncertain}),
