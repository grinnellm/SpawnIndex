...
average plant height.
\cite{HaegeleSchweigert1990} provide a description of the sampling technique,
and the basis for estimating the total number of eggs per plant.
...
the percentage of the quadrat covered by the algae and number of egg layers.

\section{CONVERT EGGS TO BIOMASS}\label{secProd}

After estimating the number of eggs in a spawn by survey type $x$,
...
$\phi_{i'qtxsnry}$ is the proportion of substrate $i'$ covered by spawn, and
...
For spawn index calculations up to this point,
subscript notation has been nested and hierarchical
(e.g., substrate type $i$ is nested within sample $j$
for surface spawn calculations in \autoref{secSurf}).
Recall from \autoref{secProt} that
the finest scale that we calculate the spawn index is
for a given survey type $x$ and spawn $s$
(i.e., $B_{xsnry}'$).
...
Thus, we sum over survey type $x$ to calculate
...
may reduce the uncertainty in $q$.
...
\end{equation}
where $B_{gry}'$ is the relative biomass for fishery $g$
from \autoref{eqSOKBioGRY} and
...
to improve spawn index accuracy:
...
to \Sexpr{max(under_width_facs$Year)}.
...
We believe that a change in lead line manufacturing
...
we adjust spawn width estimates based on written instructions
...
We correct understory spawn width by
multiplying the observed width on transect $t$ by a correction factor
