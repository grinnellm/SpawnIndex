...
the Macrocystis bed length is $L_{s_{nry}}'$ in metres.
If $L_{s_{nry}}'$ is inadvertently not recorded,
we set $L_{s_{nry}}'$ to the spawn length $L_{s_{nry}}$.
...
B'_{s_{nry}} = \frac{\mean{\rho_{s_{nry}}} L_{s_{nry}}' \mean{W_{s_{nry}}} 10^3} {\theta},
\end{equation}
