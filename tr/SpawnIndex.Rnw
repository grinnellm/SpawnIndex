...
during exceptional circumstances (e.g., the COVID-19 pandemic).
...
% TODO: SH - Relate this back to the discussion on $q$ and missed spawns.
...
The most direct is to estimate the quantity of eggs removed from the population, and determine the biomass of \fishName{} that produced those eggs.
...
This document is meant to accompany the \texttt{SpawnIndex} package, which implements the calculations described here.
