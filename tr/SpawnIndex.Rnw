...
\mean{Q_s'} = \frac{1}{T} \sum_{t=1}^{T} Q'_{ts},
\end{equation}
where $Q'_{ts}$ is the number of potential quadrats in transect $t$ (i.e., a function of spawn width).
Then, we calculate mean egg density in eggs per square metre for spawn $s$
\begin{equation}\label{eqStatRhoS}
\mean{\rho_s} = \frac{1}{T\mean{Q_s'}}\sum_{t=1}^T Q'_{ts} \mean{\rho_{ts}},
\end{equation}
...
\end{equation}
where $E_{ijs}$ is the number of egg layers in substrate $i$ from \autoref{eqSurfLayersIJS}.
...
$E_{js}$ is the total number of egg layers in sample $j$ from \autoref{eqSurfLayersJS}, and
...
dat <- as_tibble(df) %>%
  mutate(EggDens = egg_dens_surf(egg_layers = Layers))
# Figure
...
# Make a table of egg layers, proportion, and egg density
dat <- as_tibble(df) %>%
  mutate(EggDens = egg_dens_under_sub(
...
tab_cap <- paste("Spawn width correction factors $\\tau_{ry}$ for \\fishName{}
                 ...
                 and year $y$. Legend: ",
                 paste_nicely(c(major_regions, minor_regions)), ".", sep = "")
# Short caption
