...
but are considered incomplete for indexing purposes prior to 1937
...
First, the database has been used for various purposes over the last two decades
and has incidental calculations that make it overly complex.
Second, the database is difficult to troubleshoot because
it is hard to differentiate between input (i.e., data) and derived values.
...
For example, users can run \textbf{R} package functions
to implement calculations described in this document
using a small set of actual observations.
...
Finally, an \textbf{R} package will allow us to generate dynamic documents
...
Giant kelp, Macrocystis (\emph{Macrocystis} spp.),
...
A break in egg deposition is determined by the absence of \fishName{} spawn on two consecutive transects,or
...
in \fishName{} stock assessment models \citeyearpar[DFO][]{DFO2020}.
...
Although $q$ is highly uncertain
and a number of assumptions are made within the assessment model,
there are differences between the proportion of spawn observed in the two spawn survey periods \citeyearpar[DFO][]{DFO2020}.
...
in which operators collect roe that adhere to tree boughs;
...
The SpawnIndex package contains functions to import tables from the database
and calculate the spawn index.
