...
There are three types of spawn survey observation types:
...
Third, we calculate the spawn index for each spawn survey type:
...
Deuxièmement, nous développons un facteur de conversion pour convertir les oeufs de hareng du Pacifique en biomasse, ce qui est essentiel aux calculs de l'indice de frai.
...
Enfin, nous combinons les indices de frai des trois types d'observations de relevés, et nous les agrégeons par région d'évaluation des stocks et par année pour produire un indice relatif de la biomasse de frai de sexe combiné.
Nous fournissons également une approche pour calculer la biomasse pour les pêches de frai sur varech.
En outre, nous identifions les incertitudes dans les calculs de l'indice de frai, suggérons des recherches futures pour améliorer les calculs de l'indice de frai, et énumérons les mises en garde concernant l'utilisation de cet indice dans les analyses.
Enfin, nous décrivons comment les utilisateurs peuvent installer le paquet \textbf{R} SpawnIndex pour calculer l'indice de frai en utilisant une base de données d'exemple.
Bien que nous transformions les données de l'enquête sur le frai de la densité des oeufs en biomasse en tonnes, la série chronologique qui en résulte est un indice relatif de la biomasse de frai.}
