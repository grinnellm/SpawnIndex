...
\rho_{qts} = \varphi E_{qts} \phi_{qts}
\end{equation}
...
$\rho_{qts}$ is substrate egg density in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ (\autoref{figUnderDensSubQTS}).
...
\rho_{vqts} = \vartheta E^{v^\varrho}_{vqts} \phi^{v^\varsigma}_{vqts} C_v
\end{equation}
...
\rho_{qts} = \sum_{v=1}^{V} \rho_{vqts}
\end{equation}
where $\rho_{vqts}$ is egg density on algae $v$ from \autoref{eqUnderDensAlgVQTS}, and
...
\rho_{qts} = \rho_{qts} + \rho_{qts}
\end{equation}
where $\rho_{qts}$ is substrate egg density from \autoref{eqUnderDensSubQTS},
$\rho_{qts}$ is algae egg density from \autoref{eqUnderDensAlgQTS}, and
$\rho_{qts}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$.
