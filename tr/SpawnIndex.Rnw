...
There are a few caveats to keep in mind when interpreting the \fishName{} spawn index, and using spawn index data in analyses:
\begin{enumerate}
\item The spawn index a relative index of spawning biomass,
\item The spawn index is presence only;
the absence of spawn index data does not indicate the absence of spawn,
\item There are two different survey periods with substantial differences in survey effort and method (surface period from \Sexpr{min(yrRange)} to \Sexpr{newSurvYr-1}, and dive period from \Sexpr{newSurvYr} to present; \autoref{secSurfProt}),
\item Surface surveys use two different methods to estimate the number of egg layers (\autoref{secIntensity}):
\begin{enumerate}
\item Spawn intensity categories (five categories from \Sexpr{min(yrRange)} to \Sexpr{cat9Yr-1}, and nine categories from \Sexpr{cat9Yr} to \Sexpr{directEggYr-1}), and
\item Direct estimates from \Sexpr{directEggYr} to present, and
\end{enumerate}
