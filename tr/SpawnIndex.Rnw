...
\mean{\rho}_{\cdot txsnry} =
\frac{1}{Q_{txsnry}} \sum_{q=1}^{Q_{txsnry}} \rho_{qtxsnry},
\end{equation}
where $\rho_{qtxsnry}$ is egg density in thousands of eggs per square metre (m)
($10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$; \autoref{tabNotationStat}),
$Q_{txsnry}$ is the number of quadrats, and
$\mean{\rho}_{\cdot txsnry}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
Throughout this report,
`$\cdot$' represents a summation over the index that it replaces
(e.g., summation over $q$ in \autoref{eqStatRhoT}) and
`$\mean{\phantom{x}}$' represents a mean
...
\end{equation}
where $Q_{txsnry}'$ is the number of potential quadrats
(i.e., a function of spawn width) and
$T_{xsnry}$ is the number of transects.
Throughout this report,
`$'$' indicates related variables
(e.g., $Q'$ is related to $Q$).
Then, we calculate mean egg density in eggs per square metre
for spawn survey type $x$ in spawn $s$
\begin{equation}\label{eqStatRhoS}
\mean{\rho}_{\cdot xsnry} =
\frac{1}{T_{xsnry} \mean{Q}_{\cdot xsnry}'}
\sum_{t=1}^{T_{xsnry}} Q_{txsnry}' \mean{\rho}_{\cdot txsnry},
\end{equation}
where $T_{xsnry}$ is the number of transects,
$\mean{Q}_{\cdot xsnry}'$ is the mean number of potential quadrats
from \autoref{eqStatQSPrime},
$Q_{txsnry}'$ is the number of potential quadrats,
$\mean{\rho}_{\cdot txsnry}$ is the mean transect egg density from \autoref{eqStatRhoT}, and
$\mean{\rho}_{\cdot xsnry}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
...
\frac{\left(Q_{txsnry}'\mean{\rho}_{\cdot txsnry}
-\mean{Q}_{snry}'\mean{\rho}_{snry}\right)^2}
{\mean{Q}_{snry}'^2\left(T_{xsnry}-1\right)} \\
...
\end{multline}
where $T'$ is the number of potential transects in spawn $s$
(i.e., a function of spawn length),
$\mean{\rho}_{\cdot txsnry}$ is the mean transect egg density
from \autoref{eqStatRhoT},
$\mean{\rho_s}$ is the mean spawn egg density from \autoref{eqStatRhoS},
$f_{xsnry}'$ is the transect sampling fraction for spawn $s$
...
\frac{1}{{Q_{txsnry} - 1}}
\sum_{q=1}^{Q_{txsnry}}
\left(\rho_{qtxsnry} - \mean{\rho}_{\cdot txsnry}\right)^2,
\end{equation}
...
B_{\cdot ry}' = \sum_{n=1}^{N_{ry}} \sum_{s=1}^{S_{nry}} B_{\cdot snry}',
\end{equation}
where $B_{\cdot snry}'$ is the total spawn index from \autoref{eqTotIndSLRY} and
$B_{\cdot ry}'$ is a relative index of combined sex spawning biomass
for SAR $r$ and year $y$ in tonnes.

\subsection{SCALED BIOMASS}\label{secScaled}

We use $B_{\cdot ry}'$ as an indicator of relative biomass
in \fishName{} stock assessment models \citeyearpar[DFO][]{DFO2021}.
We scale $B_{\cdot ry}'$ to biomass
\begin{equation}\label{eqAbundRY}
B_{\cdot ry} = \frac{B_{\cdot ry}'} {q_{ry}},
\end{equation}
where $B_{\cdot ry}'$ is the total spawn index in tonnes from \autoref{eqTotIndRY},
$q_{ry}$ is the spawn index scaling parameter \citeyearpar[DFO][]{DFO2016}, and
$B_{\cdot ry}$ is scaled biomass for SAR $r$ and year $y$ in tonnes.
