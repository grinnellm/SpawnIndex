...
$Q_{ts}'$ is the number of potential quadrats in transect $t$,
...
B_s' = \frac{\mean{\rho_s} L_s'' \mean{W_s} 10^3} {\theta},
\end{equation}
...
\end{equation}
where $B_{xsnry}'$ is the spawn index for surface, Macrocystis, and understory spawn surveys from \autoref{eqSurfIndS}, \autoref{eqMacroIndS}, and \autoref{eqUnderIndS}, respectively, and
...
We use $B_{ry}'$ as an indicator of relative population abundance (i.e., biomass) in \fishName{} stock assessment models \citeyearpar[DFO][]{DFO2020}.
...
\end{equation}
where $W_{txsnry}'$ is the observed spawn width for transect $t$ in spawn survey type $x$ (i.e., understory), spawn $s$, Location $n$, SAR $r$, and year $y$,
