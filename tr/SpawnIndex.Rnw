...
\rho^v_{vqts} = \beta E^{v^\gamma}_{vqts} \phi^{v^\delta}_{vqts} C_v
\end{equation}
where $\beta$ is the regression slope,
$E^v_{vqts}$ is the number of egg layers on algae $v$,
$\gamma$ is the regression exponent on $E^v_{vqts}$,
$\phi^v_{vqts}$ is the proportion of quadrat $q$ covered by algae $v$,
$\delta$ is the regression exponent on $\phi^v_{vqts}$,
...
<<UnderDensAlgVQTS, fig.width=6, fig.asp=1, fig.lp="fig", echo=FALSE, fig.cap=figCap, fig.scap="Algae egg density for \\fishName{} underwater spawn surveys">>=
# Caption
...
Prediction models include \autoref{eqSurfDensJS}, \autoref{eqMacroEggsPerPlantS}, \autoref{eqUnderDensSubQTS}, and \autoref{eqUnderDensAlgVQTS}.
