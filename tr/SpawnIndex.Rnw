...
B_{ry}' = \sum_{g=1}^{G_{ry}} B_{g_{ry}}',
\end{equation}
where $B_g'$ is the relative biomass for fishery $g$ from \autoref{eqSOKBioGRY} and
$B_{ry}'$ is in tonnes.
Thus, $B_{ry}'$ is the relative biomass of \fishName{} that produced eggs
