...
the surface survey period
($q_1$; \Sexpr{pars$years$assess} to \Sexpr{pars$years$dive-1}), and
a second one for the dive survey period
($q_2$; \Sexpr{pars$years$dive} to present), for each SAR.
