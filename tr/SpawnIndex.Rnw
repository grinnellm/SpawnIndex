...
get_spawn <- function(fn) {
  ...
  if( is_data ) {
    ...
    dat <- read_csv(
      ...
      write_csv(path = here(tr_path, cache_path, fn))
  } # End if no data
  # Wrangle the data
  res <- dat %>%
    group_by(
    ...
    mutate(Index = Surface + Macrocystis + Understory) %>%
    select(
      Region, Year, StatisticalArea, Section, LocationCode, LocationName,
...
# Get spawn data
all_spawn <- get_spawn(fn = "all_spawn.csv")
...
are standardized to $20\,\text{m}$ segments (i.e., a `chain').
...
We think that a change in manufacturing prevents new lead lines from shrinking.
...
Accordingly, our best estimate of years impacted by
