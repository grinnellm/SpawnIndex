...
we calculate separate scalars for each survey period \citeyearpar[DFO][]{DFO2020}.
That is to say, we estimate one spawn index scaling parameter $q_1$ for the surface survey period, and
a second spawn index scalign parameter $q_2$ for the dive survey period.
