...
At the transect level (\autoref{figFlow}, step \ref{uTrans}), the mean \emph{linear} understory egg density in transect $t$ and spawn $s$ is
...
We calculate a weighted mean egg density because spawn width varies along the spawn length;
...
Note that some SARs continued to use old lead lines in \Sexpr{max(underWidthFac$Year)}.
...
Accordingly, our best estimate of years impacted by marking lead lines at $1.15\,\text{m}$ increments (when shrinking no longer occurred) is from \Sexpr{min(underWidthFac$Year)} and \Sexpr{max(underWidthFac$Year)}.
