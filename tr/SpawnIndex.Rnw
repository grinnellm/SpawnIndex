...
# Get spawn data
spawn_all <- read_csv(
  file = file.path("..", "..", "FIND", "Data", "FIND.csv"), col_types = cols()
) %>%
  group_by(
...
ggsave(
  filename = file.path("knitr-figs", "MapSection.png"), dpi = 600, width = 2,
  height = 2 * xy_max
)
...
ggsave(
  filename = file.path("knitr-figs", "MapStatArea.png"), dpi = 600, width = 2,
  height = 2 * xy_max
)
# Plot SAR
...
ggsave(
  filename = file.path("knitr-figs", "MapRegion.png"), dpi = 600, width = 2,
  height = 2 * xy_max
)
@
