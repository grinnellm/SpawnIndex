...
<<Intensity>>=
# Caption
tabCap <- paste("Spawn intensity categories, description, and number of egg layers for \\fishName{} surface spawn surveys for the periods ", firstYr, " to ", cat9Yr - 1, ", and ", cat9Yr, " to ", directEggYr - 1, " \\citep{HayKronlund1987, SchweigertStocker1988}. Uncertainty for number of egg layers is not available.", sep = "")
# Short caption
...
<<AlgaeCoefs>>=
# Caption
tabCap <- "Algae (aka vegetation) types $v$ and coefficients $C$ for \\fishName{} understory spawn surveys \\citep{Schweigert2005}. Uncertainty for algae coefficients is not available."
# Short caption
...
In addition, some of these studies had a limited number of samples, as well as limited spatial or temporal range, or both.
...
\caption[Details for parameters and models used in \fishName{} spawn index calculations]
{Details for studies to quantify parameters and models used in \fishName{} spawn index calculations.
Legend: Haida Gwaii (HG), Prince Rupert District (PRD), Central Coast (CC), Strait of Georgia (SoG), West Coast of Vancouver Island (WCVI),
equation (Eq), sample size $n$, standard error (SE), standard deviation (SD), spawn-on-kelp (SOK), and not available (NA).} \\
\toprule
...
$\alpha$, $\beta$ & Parameters for surface egg density model (Eq \ref{eqSurfDensJS}). & Coastwide from 1976 to 1987 ($n=$ 5,111 samples). & NA & \cite{SchweigertEtal1997} \\
...
$\alpha$ & Parameter for substrate egg density model (Eq \ref{eqUnderDensSubQTS}). & SoG in 1976, 1977, and 1978; PRD, CC, and WCVI in 1977. & NA & \cite{HaegeleHumphreys1976, HaegeleHumphreys1978a, HaegeleHumphreys1978b, HaegeleEtal1979} \\
...
$\nu$ & Proportion of SOK product that is eggs, not kelp (Eq \ref{eqSOKBioFRY}). & HG in 1983 ($n=10$ open ponds). & NA & \cite{ShieldsEtal1985} \\
$\upsilon$ & Proportion of SOK product that is eggs after brining (Eq \ref{eqSOKBioFRY}). & SoG. & NA & \cite{WhyteEnglar1977} \\
