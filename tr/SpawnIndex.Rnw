...
First, we develop a statistical framework (\autoref{secStat}) and sampling protocol (\autoref{secProt}) to estimate the number of eggs in a given area.
...
Finally, we combine the three spawn indices, and aggregate by SAR and year to produce a relative index of combined sex spawning biomass (\autoref{secTotal}).
