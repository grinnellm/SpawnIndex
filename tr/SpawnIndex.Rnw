...
\end{enumerate}
\item Investigate surface spawn width adjustments (e.g,, mean vs median width, annual vs periodic updates; \autoref{secWidthSurface}).
\end{enumerate}
...
It is now standard practice to re-measure transect segments annually to ensure that this issue does not occur egain.
