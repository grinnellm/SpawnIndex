...
\rho_{\mean{q}txsnry} = \frac{1}{Q_{txsnry}} \sum_{q=1}^{Q_{txsnry}} \rho_{qtxsnry},
\end{equation}
where $\rho_{qtxsnry}$ is egg density in thousands of eggs per square metre (m)
for quadrat $q$ ($10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$;
\autoref{tabNotationStat}),
$Q_{txsnry}$ is the number of quadrats, and
$\rho_{\mean{q}txsnry}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
...
Q_{\mean{t}xsnry}' = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} Q_{txsnry}',
\end{equation}
...
\mean{\rho_{snry}} = \frac{1}{T_{xsnry}Q_{\mean{t}xsnry}'}\sum_{t=1}^{T_{xsnry}} Q_{txsnry}' \rho_{\mean{q}txsnry},
\end{equation}
where $T_{xsnry}$ is the number of transects,
$Q_{\mean{t}xsnry}'$ is the mean number of potential quadrats
from \autoref{eqStatQSPrime},
$Q_{txsnry}'$ is the number of potential quadrats,
$\rho_{\mean{q}txsnry}$ is the mean transect egg density from \autoref{eqStatRhoT}, and
...
\frac{\left(Q_{txsnry}'\rho_{\mean{q}txsnry}-\mean{Q_{snry}'}\mean{\rho_{snry}}\right)^2}
{\mean{Q_{snry}'}^2\left(T_{xsnry}-1\right)} \\
+ \frac{f_{txsnry}}{T_{xsnry}^2} \sum_{t=1}^{T_{xsnry}}
\left(\frac{Q_{txsnry}'}{\mean{Q_{snry}'}}\right)^2
\frac{\left(1-f_{qtxsnry}\right)\sigma_{txsnry}^2}{Q_{txsnry}},
\end{multline}
...
\left(\rho_{qtxsnry} - \rho_{\mean{q}txsnry}\right)^2,
\end{equation}
where $\rho_{q}$ is egg density for quadrat $q$, and
$\rho_{\mean{q}txsnry}$ is the mean transect egg density
...
from \autoref{eqSurfLayersIJS},
$E_{\cdot jxsnry}$ is in number of egg layers, and
`$\cdot$' represents a summation over the index that it replaces
(e.g., summation over $i$ in \autoref{eqSurfLayersJS}).
...
$J_{xsnry}$ is the number of samples,
$\rho_{\mean{j}xsnry}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$, and
`$\mean{\phantom{x}}$' represents a mean of the index that it covers
(e.g., mean of $j$ in \autoref{eqSurfDensS}).
