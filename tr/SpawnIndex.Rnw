...
Fecundity could be influenced by biological processes such as the observed non-stationarity of weight-at-age \citeyearpar[DFO][]{DFO2020}, or a truncated age distribution caused by selective fishing or high natural mortality.
...
% This section is very similar to the previous one on Uncertainty -- combine them together somehow, or make them more distinct.
...
In addition, parameter and prediction model uncertainty could be propagated through the calculations to quantify spawn index uncertainty (\autoref{secUncertain}).
...
In addition to work mentioned here and in the previous section, other future work could include:
\begin{enumerate}
...
\end{enumerate}
\item Investigate surface spawn width adjustments (e.g,, use mean instead of median, update annually; \autoref{secWidthSurface}).
\end{enumerate}
