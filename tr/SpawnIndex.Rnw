...
Note that we use subsections to separate levels of spatial aggregation within each section of this report (e.g., calculations at the quadrat, or transect level; \autoref{figFlow}).
Finally, we combine the spawn indices from the three types of suvery observations, and
...
We set $W_s$ to the first non-missing value of median pool width, median Section width, median SAR width, or observed width $W'$ (in that order; \autoref{secWidthSurface}).
