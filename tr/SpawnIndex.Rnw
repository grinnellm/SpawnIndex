...
# Section extent ratio
xy_sec <- get_ext_ratio(dat = filter(sf_sec, Section == zoom_sec))
...
map_sec <- ggplot(data = bc_coast) +
  ...
  labs(x = NULL, y = NULL, colour = "Spawn index (t)") +
  guides(
...
ggsave(
  filename = here("tr", fig_path, "MapSection.png"), dpi = 600, width = 2,
  height = 2 * xy_max
)
...
ggsave(
  filename = here("tr", fig_path, "MapStatArea.png"), dpi = 600, width = 2,
  height = 2 * xy_max
)
# Plot SAR
...
ggsave(
  filename = here("tr", fig_path, "MapRegion.png"), dpi = 600, width = 2,
  height = 2 * xy_max
)
@
