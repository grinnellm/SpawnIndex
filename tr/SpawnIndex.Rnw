...
This document accompanies our SpawnIndex package (\autoref{secPack}).
...
The SpawnIndex \textbf{R} \citeyearpar[RCT][]{R-4.0.3-32} package is publicly accessible on the \gitRepo{}.
...
Therefore, we exclude spawn data prior to \Sexpr{pars$years$assess} from stock assessments.
