...
\mean{Q}_{\cdot xsnry}' = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} Q_{txsnry}',
\end{equation}
...
\frac{\left(Q_{txsnry}'\rho_{\mean{q}txsnry}
-\mean{Q_{snry}'}\mean{\rho_{snry}}\right)^2}
{\mean{Q_{snry}'}^2\left(T_{xsnry}-1\right)} \\
+ \frac{f_{xsnry}'}{T_{xsnry}^2} \sum_{t=1}^{T_{xsnry}}
\left(\frac{Q_{txsnry}'}{\mean{Q_{snry}'}}\right)^2
...
$\rho_{\mean{q}txsnry}$ is the mean transect egg density from \autoref{eqStatRhoT},
...
\sigma_{txsnry}^2 = \frac{1}{{Q_{txsnry}-1}} \sum_{q=1}^{Q_{txsnry}}
...
\rho_{\mean{j}xsnry} = \frac{1}{J_{xsnry}} \sum_{j=1}^{J_{xsnry}} \rho_{jxsnry},
\end{equation}
where $\rho_{jxsnry}$ is egg density in sample $j$ from \autoref{eqSurfDensJS},
$J_{xsnry}$ is the number of samples, and
$\rho_{\mean{j}xsnry}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
...
B_{xsnry}' = \frac{\rho_{\mean{j}xsnry} L_{xsnry} W_{xsnry} 10^3} {\theta},
\end{equation}
where $\rho_{\mean{j}xsnry}$ is mean egg density in spawn $s$ from \autoref{eqSurfDensS},
...
mean height $\mean{H_{txsnry}}$ in metres, and
mean number of egg layers $\mean{E_{txsnry}}$.
...
\mean{E_{xsnry}} = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} \mean{E_{txsnry}},
\end{equation}
where $\mean{E_t}$ is the mean number of egg layers in transect $t$,
$T$ is the number of transects in spawn $s$, and
$\mean{E_x}$ is in number of egg layers.
Second, the mean plant height in spawn $s$ is
\begin{equation}\label{eqMacroHeightS}
\mean{H_{xsnry}} = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} \mean{H_{txsnry}},
\end{equation}
...
\mean{\kappa_{xsnry}} = \frac{K_{xsnry}}{P_{xsnry}},
\end{equation}
...
and $\mean{\kappa_x}$ is in number of stalks per plant.
...
\mean{\psi_{xsnry}} = \xi \mean{E_{xsnry}}^\gamma \mean{H_{xsnry}}^\delta \mean{\kappa_{xsnry}}^\epsilon 10^3,
\end{equation}
where $\xi$ is the regression slope,
$\mean{E_x}$ is the mean number of egg layers in spawn $s$ from \autoref{eqMacroLayersS},
$\gamma$ is the regression exponent on $\mean{E_x}$,
$\mean{H_x}$ is the mean plant height in spawn $s$
from \autoref{eqMacroHeightS},
$\delta$ is the regression exponent on $\mean{H_x}$,
$\mean{\kappa_x}$ is the mean number of stalks per plant in spawn $s$
from \autoref{eqMacroStalksPerPlantS},
$\epsilon$ is the regression exponent on $\mean{\kappa_x}$, and
$\mean{\psi_x}$ is in $\text{eggs} \cdot 10^3 \cdot \text{plant}^{-1}$ (\autoref{figMacroEggsPerPlantS}).
Mean Macrocystis egg density in spawn $s$ is
\begin{equation}\label{eqMacroDensS}
\mean{\rho_{xsnry}} = \frac{\mean{\psi_{xsnry}} P_{xsnry}}{A_{xsnry}},
\end{equation}
where
$\mean{\psi_x}$ is the mean number of eggs per plant in spawn $s$ from \autoref{eqMacroEggsPerPlantS},
$P_x$ is the number of plants in spawn $s$ from \autoref{eqMacroPlantsS},
$A_x$ is the total area of transects in spawn $s$ from \autoref{eqMacroAreaS}, and
$\mean{\rho_x}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.

The Macrocystis spawn index in spawn $s$ is
\begin{equation}\label{eqMacroIndS}
B_{xsnry}' = \frac{\mean{\rho_{xsnry}} L_{xsnry}' \mean{W_{xsnry}} 10^3} {\theta},
\end{equation}
where $\mean{\rho_x}$ is the mean egg density in spawn $s$ from \autoref{eqMacroDensS},
$L_x'$ is the length of the Macrocystis bed in spawn $s$,
$\mean{W_x}$ is the mean width of spawn $s$ from \autoref{eqMacroWidthS},
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
...
\mean{\rho_{txsnry}} = \frac{1}{Q_{txsnry}} \sum_{q=1}^{Q_{txsnry}} \rho_{qtxsnry},
\end{equation}
...
\mean{W_{xsnry}} = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} W_{txsnry},
\end{equation}
where $W_t$ is the spawn width at transect $t$,
$T$ is the number of transects in spawn $s$, and
$\mean{W_x}$ is in metres.
...
\mean{\rho_{xsnry}} = \frac{\sum_{t=1}^{T_{xsnry}} \mean{\rho_{txsnry}} W_{txsnry}}
{\sum_{t=1}^{T_{xsnry}} W_{txsnry}},
\end{equation}
where $\mean{\rho_t}$ is the mean understory egg density in transect $t$
