...
This written document is meant to accompany our SpawnIndex (\autoref{secDown}) \textbf{R} \citeyearpar[RCT][]{R-3.6.1-32} package.
The SpawnIndex package has all the calculations to calculate the spawn index,
and is another step towards following `good enough' practices in scientific computing \citep{WilsonEtal2016}.
Previously, spawn index calculations were implemented in a \textbf{Microsoft Access} database, and were practically documented.
...
$i_{jxsnry}$ & Substrate type (surface) & 1, 2, 3, ..., $I_{jxsnry}$ \\
...
$i_{qtxsnry}'$ & Substrate type (understory) & 1, 2, 3, ..., $I_{jxsnry}$ \\
...
$\phi_i$ is the proportion of substrate $i$ covered by spawn, and
...
\end{equation}
where $\rho_i$ is substrate egg density from \autoref{eqUnderDensSubQTS},
...
$T$ is the number of transects in spawn $s$, and
$\mean{W_s}$ is in metres.
The algae bed length is $L_{s_{nry}}''$, also in metres.
