...
\mean{\rho_{t_{xsnry}}} = \frac{1}{Q_{txsnry}} \sum_{q=1}^{Q_{txsnry}} \rho_{q_{txsnry}},
\end{equation}
...
\mean{\rho_{s_{nry}}} = \frac{1}{T_{xsnry}\mean{Q_{xsnry}'}}\sum_{t=1}^{T_{xsnry}} Q_{txsnry}' \mean{\rho_{t_{xsnry}}},
\end{equation}
...
+ \frac{f_{t_{xsnry}}}{T_{xsnry}^2} \sum_{t=1}^{T_{xsnry}}
...
\sigma_{t_{xsnry}}^2 = \frac{1}{{Q_{txsnry}-1}} \sum_{q=1}^{Q_{txsnry}}
...
(e.g., $\rho_{\mean{j}xsnry}$ is the mean of the $\rho_{jxsnry}$ values).
...
K_{t_{xsnry}} = \sum_{p=1}^{P_{txsnry}} K_{p_{txsnry}},
\end{equation}
...
\mean{E_{{xs}_{nry}}} = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} \mean{E_{t_{xsnry}}},
\end{equation}
where $\mean{E_t}$ is the mean number of egg layers in transect $t$,
$T$ is the number of transects in spawn $s$, and
$\mean{E_{xs}}$ is in number of egg layers.
Second, the mean plant height in spawn $s$ is
\begin{equation}\label{eqMacroHeightS}
\mean{H_{{xs}_{nry}}} = \frac{1}{T_{xsnry}} \sum_{t=1}^{T_{xsnry}} \mean{H_{t_{xsnry}}},
\end{equation}
where $\mean{H_t}$ is the mean plant height in transect $t$,
$T$ is the number of transects in spawn $s$, and
$\mean{H_{xs}}$ is in metres.
...
K_{{xs}_{nry}} = \sum_{t=1}^{T_{xsnry}} K_{t_{xsnry}},
\end{equation}
where $K_t$ is the number of stalks in transect $t$
from \autoref{eqMacroStalksTS} and
$K_{xs}$ is in number of stalks.
The total number of observed plants in spawn $s$ is
\begin{equation}\label{eqMacroPlantsS}
P_{{xs}_{nry}} = \sum_{t=1}^{T_{xsnry}} P_{t_{xsnry}},
\end{equation}
where $P_t$ is the number of plants in transect $t$ and
$P_{xs}$ is in number of plants.
Thus, the mean number of stalks per plant in spawn $s$ is
\begin{equation}\label{eqMacroStalksPerPlantS}
\mean{\kappa_{{xs}_{nry}}} = \frac{K_{{xs}_{nry}}}{P_{{xs}_{nry}}},
\end{equation}
where $K_{xs}$ is the number of stalks in spawn $s$ from \autoref{eqMacroStalksS},
$P_{xs}$ is the number of plants in spawn $s$ from \autoref{eqMacroPlantsS},
and $\mean{\kappa_{xs}}$ is in number of stalks per plant.
...
\mean{\psi_{{xs}_{nry}}} = \xi \mean{E_{{xs}_{nry}}}^\gamma \mean{H_{{xs}_{nry}}}^\delta \mean{\kappa_{{xs}_{nry}}}^\epsilon 10^3,
\end{equation}
where $\xi$ is the regression slope,
$\mean{E_{xs}}$ is the mean number of egg layers in spawn $s$ from \autoref{eqMacroLayersS},
$\gamma$ is the regression exponent on $\mean{E_{xs}}$,
$\mean{H_{xs}}$ is the mean plant height in spawn $s$
from \autoref{eqMacroHeightS},
$\delta$ is the regression exponent on $\mean{H_{xs}}$,
$\mean{\kappa_{xs}}$ is the mean number of stalks per plant in spawn $s$
from \autoref{eqMacroStalksPerPlantS},
$\epsilon$ is the regression exponent on $\mean{\kappa_{xs}}$, and
$\mean{\psi_{xs}}$ is in $\text{eggs} \cdot 10^3 \cdot \text{plant}^{-1}$ (\autoref{figMacroEggsPerPlantS}).
Mean Macrocystis egg density in spawn $s$ is
\begin{equation}\label{eqMacroDensS}
\mean{\rho_{{xs}_{nry}}} = \frac{\mean{\psi_{{xs}_{nry}}} P_{{xs}_{nry}}}{A_{{xs}_{nry}}},
\end{equation}
where
$\mean{\psi_{xs}}$ is the mean number of eggs per plant in spawn $s$ from \autoref{eqMacroEggsPerPlantS},
$P_{xs}$ is the number of plants in spawn $s$ from \autoref{eqMacroPlantsS},
$A_{xs}$ is the total area of transects in spawn $s$ from \autoref{eqMacroAreaS}, and
$\mean{\rho_{xs}}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.

The Macrocystis spawn index in spawn $s$ is
\begin{equation}\label{eqMacroIndS}
B_{{xs}_{nry}}' = \frac{\mean{\rho_{{xs}_{nry}}} L_{{xs}_{nry}}' \mean{W_{{xs}_{nry}}} 10^3} {\theta},
\end{equation}
where $\mean{\rho_{xs}}$ is the mean egg density in spawn $s$ from \autoref{eqMacroDensS},
$L_{xs}'$ is the length of the Macrocystis bed in spawn $s$,
$\mean{W_{xs}}$ is the mean width of spawn $s$ from \autoref{eqMacroWidthS},
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B_{xs}'$ is in tonnes (step \ref{mInd} in \autoref{figFlow}).
...
\rho_{q_{txsnry}} = \rho_{i_{qtxsnry}'} + \sum_{v=1}^{V_{qtxsnry}} \rho_{v_{qtxsnry}},
\end{equation}
...
$T$ is the number of transects in spawn $s$, and
$\mean{W_{xs}}$ is in metres.
The algae bed length is $L_{xs}''$, also in metres.
...
\mean{\rho_{{xs}_{nry}}} = \frac{\sum_{t=1}^{T_{xsnry}} \mean{\rho_{t_{xsnry}}} W_{t_{xsnry}}}
{\sum_{t=1}^{T_{xsnry}} W_{t_{xsnry}}},
\end{equation}
...
B_{{xs}_{nry}}' = \frac{\mean{\rho_{{xs}_{nry}}} L_{{xs}_{nry}}'' \mean{W_{{xs}_{nry}}} 10^3} {\theta},
\end{equation}
where $\mean{\rho_{xs}}$ is the mean understory egg density from
\autoref{eqUnderDensMuS},
$L_{xs}''$ is the length of the algae bed,
$\mean{W_{xs}}$ is the mean spawn width from \autoref{eqUnderWidthMuS},
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B_{xs}'$ is in tonnes (step \ref{uInd} in \autoref{figFlow}).
