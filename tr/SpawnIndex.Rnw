...
We use $B^\prime_{ry}$ as an indicator of \fishName{} relative population abundance (i.e., biomass) in stock assessment models \citeyearpar[DFO][]{DFO2020}.
We scale $B^\prime_{ry}$ to abundance
\begin{equation}\label{eqAbundRY}
B_{ry} = \frac{B^\prime_{ry}} {q_y}
\end{equation}
where $B^\prime_{ry}$ is the total spawn index in tonnes from \autoref{eqTotIndRY},
$q_y$ is the spawn index scaling parameter, and
