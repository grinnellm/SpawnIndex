...
$v$ & Algae (aka vegetation) type & 1, 2, 3, ..., $V$ \\
...
Similarly, Macrocystis bed length $L^m$ and algae (aka vegetation) bed length $L^v$ refer to distances that Macrocystis and algae beds extend parallel to the shore, respectively.
...
To sample, surveyors deploy specialized rakes throughout the spawn to determine algae (aka vegetation) type, number of egg layers, and percent cover.
...
Understory spawn surveyors use SCUBA gear to collect underwater data for algae (aka vegetation) types $v$, quadrats $q$, transects $t$, and spawns $s$;
...
<<AlgaeCoefs>>=
# Caption
tabCap <- "Algae (aka vegetation) types $v$ and coefficients $C$ for \\fishName{} understory spawn surveys \\citep{Schweigert2005}."
# Short caption
...
We summarise spawn width by the median because widths are not normally distributed \citep{SchweigertEtal1993}.
