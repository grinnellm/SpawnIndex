...
Also note that this section pertains to the understory spawn survey protocol;
the Macrocystis spawn survey protocol is discussed below.
...
% TODO: RF - This systematic sampling approach has advantages and disadvancages
\citep{SchaefferEtal2012}
...
Giant kelp, Macrocystis (\emph{Macrocystis} spp.), requires a different sampling protocol than the aforementioned understory spawn survey protocol.
...
For example, in stock assessments, we scale the \fishName{} spawn index to abundance by dividing the index by $q$,
