...
$\rho'$ & Substrate egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
...
$\rho''$ & Algae egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$\rho$ & Egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$W$ & Spawn width & m & \\
$\mean{\rho}$ & Mean egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$\mean{W}$ & Mean spawn width & m & \\
...
\rho'_{qts} = \varphi E_{qts} \phi_{qts}
\end{equation}
where $\varphi$ is the slope,
$E_{qts}$ is the number of egg layers on substrate in quadrat $q$,
$\phi_{qts}$ is the proportion of substrate in quadrat $q$ covered by spawn, and
$\rho'_{qts}$ is substrate egg density in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ (\autoref{figUnderDensSubQTS}).
...
\rho''_{vqts} = \vartheta E^\varrho_{vqts} \phi^\varsigma_{vqts} C_v
\end{equation}
...
$\rho''_{vqts}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ (\autoref{figUnderDensAlgVQTS}).
The total algae egg density for quadrat $q$ in transect $t$ and spawn $s$ is
\begin{equation}\label{eqUnderDensAlgQTS}
\rho''_{qts} = \sum_{v=1}^{V} \rho''_{vqts}
\end{equation}
where $\rho''_{vqts}$ is egg density on algae $v$ from \autoref{eqUnderDensAlgVQTS}, and
$\rho''_{qts}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$.
...
\rho_{qts} = \rho'_{qts} + \rho''_{qts}
\end{equation}
where $\rho'_{qts}$ is substrate egg density from \autoref{eqUnderDensSubQTS},
$\rho''_{qts}$ is algae egg density from \autoref{eqUnderDensAlgQTS}, and
