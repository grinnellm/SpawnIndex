...
they used small vessels to search for spawn, and surface surveys to estimate spawn biomass.
...
% TODO: RK - I think what you mean is effort searching for spawn may take less time when over-flights are available, while the effort to measure egg deposition has increased.
...
For example, we scale the \fishName{} spawn index to abundance by dividing the index by $q$,
...
That is to say, we estimate one spawn index scaling parameter for
