...
\cite{HartTester1934} first demonstrated that an estimate of \fishName{} biomass could be determined by counting egg deposition in a small set of sampling quadrats.
...
requires a different framework than the understory spawn framework using quadrats.
...
surveyors deploy specialized rakes throughout the spawn to determine algae (i.e., vegetation) type, number of egg layers, and percent cover.
