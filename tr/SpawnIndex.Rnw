...
\begin{figure}
\centering
\begin{tikzpicture}[node distance=1.0cm and 0.2cm, font=\footnotesize]
...
To facilitate pairing this document with the \textbf{R} package,
we cross-reference equations and functions (\autoref{secCross}).
...
\section{CROSSWALK TABLE}\label{secCross}
...
functions in the \textbf{R} package (\autoref{tabLookupFuns}).
In addition, most parameters names in this document have consistent
function argument names in the \textbf{R} package
(e.g., theta $\theta$ in \autoref{eqECF} is
\texttt{theta} in the corresponding function \texttt{eggs\_to\_sb}).
% Second, we cross-reference parameter in this document with
% function arguments in the \textbf{R} package (\autoref{tabLookupPars}).
...
functions in the SpawnIndex \textbf{R} package (\gitRepo{}).}
\begin{tabulary}{\linewidth}{LLLl}
\toprule
& \multicolumn{2}{c}{\textbf{Technical report}} & \multicolumn{1}{c}{\textbf{R package}} \\
\cmidrule(r){2-3} \cmidrule(l){4-4}
\textbf{Description} & \textbf{Section} & \textbf{Equation(s)} & \textbf{Function} \\
\midrule
...
% \FloatBarrier
% \begin{longtable}{P{3.5cm}P{1.25cm}P{1.75cm}P{1.5cm}P{3.5cm}}
% \caption[Crosswalk table for parameters and function arguments in the SpawnIndex \textbf{R} package]
% {Crosswalk table for parameters in this document (i.e., technical report) and
% function arguments in the SpawnIndex \textbf{R} package (\gitRepo{}).
% Legend: `Eq' indicates equation.}\\
% \toprule
% & \multicolumn{2}{c}{\textbf{Technical report}} & \multicolumn{2}{c}{\textbf{R package}} \\
% \cmidrule(r){2-3} \cmidrule(l){4-5}
% \textbf{Description} & \textbf{Name} & \textbf{Scope} & \textbf{Name} & \textbf{Scope} \\
% \midrule
% \endfirsthead
% \toprule
% \multicolumn{5}{c}{\emph{\tablename~\thetable~continued}} \\
...
% \bottomrule
% \endlastfoot
% Female fecundity & $\omega$ & \autoref{tabNotationEggs}, Eq~\ref{eqECF} & \texttt{omega} & \texttt{pars}, \texttt{eggs\_to\_sb} \\
% Proportion female & $\female$ & \autoref{tabNotationEggs}, Eq~\ref{eqECF} & \texttt{female} & \texttt{pars}, \texttt{eggs\_to\_sb} \\
% Egg conversion factor & $\theta$ & \autoref{tabNotationEggs}, Eqs~\ref{eqECF}, \ref{eqSurfIndS}, \ref{eqMacroIndS}, \ref{eqUnderIndS}, \ref{eqSOKBioGRY} & \texttt{theta} & \texttt{eggs\_to\_sb}, \texttt{calc\_surf\_index}, \texttt{calc\_macro\_index}, \texttt{calc\_under\_index}, \texttt{calc\_sok\_index} \\
% \bottomrule
% \label{tabLookupPars}
% \end{longtable}
% \FloatBarrier
