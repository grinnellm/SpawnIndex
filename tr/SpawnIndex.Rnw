...
$\female$ is proportion of spawners that are female, and
...
We estimate the biomass of \fishName{} that spawned and produced eggs which were removed from the population by SOK fishery $f$ in SAR $r$ and year $y$ as
...
\end{equation}
where $B_{fry}$ is spawning biomass for fishery $f$ from \autoref{eqSOKBioFRY}, and
