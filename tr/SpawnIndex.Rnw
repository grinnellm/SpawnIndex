...
We calculate the spawn index in four steps.
First, we develop a sampling protocol to estimate the number of eggs in a given area (\autoref{secStat}, \autoref{secProt}).
...
\node [terminal, label={[text width=9.5em] \textbf{Macrocystis (M)}}] (mExt) {\labelMacro{mExt} Determine spatial extent and transect locations (\S\ref{secMacroProt}).};
...
\node [terminal, label={[text width=9.5em] \textbf{Surface (S)}}, left=of mExt] (sExt) {\labelSurf{sExt} Determine spatial extent and sample locations (along transects if possible; \S\ref{secSurfProt}).};
...
\node [terminal, label={[text width=9.5em] \textbf{Understory (U)}}, right=of mExt] (uExt) {\labelUnder{uExt} Determine spatial extent, transect locations, and quadrat spacing (\S\ref{secUnderProt}).};
...
for surface spawn surveys that don't use transects (\autoref{secSurfProt}), surveyors collect longitude and latitude at the start and ends of the spawn (i.e., overall length and width).
...
After determining the spatial extent, surveyors determine the number of egg layers on substrate and algae according to sampling protocols described in \autoref{secSurfProt}, \autoref{secMacroProt}, \& \autoref{secUnderProt}.
