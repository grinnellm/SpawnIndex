...
For example, surface surveys are less accurate than dive surveys (\hyperref[secWidthSurface]{appendix \ref{secWidthSurface}}), and
...
to estimate mean egg density with a standard error of at most 25\%.
...
% TODO: Add details re when, where, number of samples, uncertainty, etc.
<<Intensity>>=
# Caption
...
BC \fishName{} BC in 1974 and 1980 (North Coast, Strait of Georgia, and West Coast of Vancouver Island), and
Califorina \fishName{} in 1975 \citep{RabinBarnhart1977}.
...
In this document, the term `metric' refers to a measure of quantitative assessment.
...
Three measurements are required to calculate the number of eggs on Macrocystis plants \citep{HaegeleSchweigert1990}:
...
\citet{HaegeleSchweigert1990} developed a predictive model of number of eggs per plant as a function of number of egg layers, plant height, and number of stalks per plant using a nonlinear multiple regression model
...
Understory spawn surveyors use SCUBA gear to collect underwater data for algae (aka vegetation) types $v$, quadrats $q$, transects $t$, and spawns $s$ (\autoref{secUnderProt});
...
\end{enumerate}
Despite these assumptions and potential sources of uncertainty, the spawn index has typically been reported without quantifying uncertainty (but see \citet{SchweigertEtal1993} who estimated the variance).
...
\item Incorporate the underlying data that informs the prediction model equations into spawn index calculations,
...
Quantifying uncertainty may also identify options to increase survey program efficiency by targeting data that have the largest impact on spawn index accuracy.
...
Many of the analyses to quantify parameters and prediction models used in spawn index calculations are dated (\autoref{tabUncertainty});
these analyses could be checked with new information, and updated if required.
In addition, some of these analyses have a limited number of samples, as well as limited spatial or temporal range, or both.
...
In addition to work mentioned above and in the previous section, other potential research includes:
\begin{enumerate}
\item Review the assumed statistical framework,
...
\item Investigate the assumptions that eggs on substrate and algae are:
\begin{enumerate}
\item Independent (\autoref{eqUnderDensQTS}), and
\item Represented by the same length measurement (\autoref{secUnderS}), and
\end{enumerate}
\item Investigate surface spawn width adjustments (\autoref{secWidthSurface}):
\begin{enumerate}
\item Mean vs median width,
\item Annual vs periodic updates, and
\item Consistency of spawn width over time at a range of abundance levels.
\end{enumerate}
\end{enumerate}
...
\caption[Details for parameters and models used in \fishName{} spawn index calculations]{Details for parameters and models used in \fishName{} spawn index calculations. Legend: Haida Gwaii (HG), Prince Rupert District (PRD), Central Coast (CC), Strait of Georgia (SoG), West Coast of Vancouver Island (WCVI), sample size $n$, standard error (SE), standard deviation (SD), spawn-on-kelp (SOK), and equation (Eq).} \\
\toprule
Parameters & Description & Study details & Uncertainty & References \\
\midrule
\endfirsthead
\toprule
...
\bottomrule
\endlastfoot
$f^t$, $f^q$ & Parameters for statistical framework (Eq \ref{eqStatSigmaS}). & SoG in 1981 and 1983; WCVI in 1982. & Mean egg density $\text{SE}\leq 25$\%. & \cite{SchweigertEtal1985, SchweigertEtal1990} \\
$\omega$ & Female fecundity (Eq \ref{eqECF}). & NC, WCVI, and SoG in 1974 ($n=$ 3,293 fish) and 1980 ($n=$ 1,642 fish); California in 1975 ($n=37$ fish). & SD ranges from 16.9 to 53.5. & \cite{Hay1985, HayBrett1988} \\
$\phi^f$ & Proportion female (Eq \ref{eqECF}). & & & \\
$\alpha$, $\beta$ & Parameters for surface egg density model (Eq \ref{eqSurfDensJS}). & Coastwide from 1976 to 1987 ($n=$ 5,111 samples). & & \cite{SchweigertEtal1997} \\
$\beta$, $\gamma$, $\delta$, $\epsilon$ & Parameters for number of eggs per Macrocystis plant model (Eq \ref{eqMacroEggsPerPlantS}). & HG in 1981 and 1987 ($n=112$ plants); PRD in 1986 ($n=15$ plants); CC in 1986 ($n=5$ plants); WCVI in 1985 and 1986 ($n=35$ plants). & Model accounts for 78\% of variation in eggs per plant. & \cite{HaegeleSchweigert1990} \\
$\alpha$ & Parameter for substrate egg density model (Eq \ref{eqUnderDensSubQTS}). & SoG in 1976, 1977, and 1978; PRD, CC, and WCVI in 1977. & & \cite{HaegeleHumphreys1976, HaegeleHumphreys1978a, HaegeleHumphreys1978b, HaegeleEtal1979} \\
$\beta$, $\gamma$, $\delta$ & Parameters for algae egg density model (Eq \ref{eqUnderDensAlgVQTS}). & Coastwide from 1976 to 1987 ($n=$ 5,111 samples). & Model accounts for 40\% of variation in egg density. & \cite{Schweigert2005} \\
$\nu$ & Proportion of SOK product that is eggs, not kelp (Eq \ref{eqSOKBioFRY}). & HG in 1983 ($n=10$ open ponds). & & \cite{ShieldsEtal1985} \\
$\upsilon$ & Proportion of SOK product that is eggs after brining (Eq \ref{eqSOKBioFRY}). & SoG. & & \cite{WhyteEnglar1977} \\
...
\FloatBarrier
% Range from xx to xx\%.
% HG ($n=$ 10 ponds) and PRD ($n=$ 8 ponds) in 1982;
