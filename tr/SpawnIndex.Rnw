...
\begin{landscape}
\begin{longtable}{P{1.75cm}P{5cm}P{5cm}P{3cm}P{3cm}}
% \caption{Details for parameters and models used in \fishName{} spawn index calculations. Legend: Haida Gwaii (HG), Prince Rupert District (PRD), Central Coast (CC), Strait of Georgia (SoG), West Coast of Vancouver Island (WCVI), sample size $n$, standard error (SE), standard deviation (SD), spawn-on-kelp (SOK), and equation (Eq).}
% [Details for parameters and models used in \fishName{} spawn index calculations]
\toprule
...
$\omega$ & Female fecundity (Eq \ref{eqECF}). & NC, WCVI, and SoG in 1974 and 1980 ($n$ ranges from 290 to 1,715; total $n=$ 4,935); California in 1975 ($n=37$). & SD ranges from 16.9 to 53.5. & \cite{Hay1985, HayBrett1988} \\
$\phi^f$ & Proportion female (Eq \ref{eqECF}). & & & \\
$\alpha$, $\beta$ & Parameters for surface egg density model (Eq \ref{eqSurfDensJS}). & Coastwide from 1976 to 1987 ($n=$ 5,111). & & \cite{SchweigertEtal1997} \\
$\beta$, $\gamma$, $\delta$, $\epsilon$ & Parameters for number of eggs per Macrocystis plant model (Eq \ref{eqMacroEggsPerPlantS}). & HG in 1981 and 1987 ($n=$ 112); PRD in 1986 ($n=$ 15); CC in 1986 ($n=$ 5); WCVI in 1985 and 1986 ($n=$ 35). & Model accounts for 78\% of variation in eggs per plant. & \cite{HaegeleSchweigert1990} \\
...
$\beta$, $\gamma$, $\delta$ & Parameters for algae egg density model (Eq \ref{eqUnderDensAlgVQTS}). & Coastwide from 1976 to 1987 ($n=$ 5,111). & Model accounts for 40\% of variation in egg density. & \cite{Schweigert2005} \\
$\nu$ & Proportion of SOK product that is eggs, not kelp. & & & \cite{ShieldsEtal1985} \\
$\upsilon$ & Proportion of SOK product that is eggs after brining. & & & \cite{WhyteEnglar1977} \\
$M$ & Average mass of a fertilized egg. & & & \cite{HayMiller1982}
