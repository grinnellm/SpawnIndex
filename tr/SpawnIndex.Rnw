...
Throughout this report, the subscript `$\cdot$' indicates a summation
(e.g., $E_{\cdot jxsnry}$ is the sum of the $E_{ijxsnry}$ values).
...
Throughout this report, the notation `$\mean{x}$' indicates a mean
