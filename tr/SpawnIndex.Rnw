...
Section \Sexpr{zoom_sec}, Statistical Area \Sexpr{zoom_sa}, \Sexpr{zoom_reg} SAR.}
\label{figRegionSec}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.325\linewidth}
\includegraphics[width=\linewidth]{\Sexpr{fig_path}/MapStatArea.png}
\caption{Sections (Sec) in Statistical Area \Sexpr{zoom_sa}, \Sexpr{zoom_reg} SAR.
...
\label{figRegionReg}
\end{subfigure}
\caption[\fishName{} spawn index by Location, as well as Sections and Statistical Areas in the \Sexpr{reg_name_long} stock assessment region]
{\fishName{} spawn index in \Sexpr{yr_index} by Location (\subref{figRegionSec}), as well as Sections (\subref{figRegionSA}) and Statistical Areas (\subref{figRegionReg}) in the \Sexpr{reg_name_long} (\Sexpr{zoom_reg}) stock assessment region (SAR; \autoref{figBC}).}
\label{figRegion}
\end{figure}
...
{Sequence of steps (e.g., \ref{sExt}) for \fishName{} spawn index calculations for spawn survey observation type $x = \{\text{surface, Macrocystis, understory}\}$ (\autoref{tabNotationIndices}).
...
Legend: metres (m).
Note that `$\,'$' indicates related variables (e.g., $f'$ is related to $f''$).}
\begin{tabulary}{\linewidth}{lLL}
\toprule
...
by SOK fishery $g$ (\autoref{tabNotationIndices}) as
...
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B_{gry}'$ is in tonnes (\autoref{tabNotationSOK}).
...
some SARs and years had all $1.0\,\text{m}$ increment lengths
...
$W_{txsnry}$ is in metres (\autoref{tabNotationWidth}).
