...
$E_i$ is the number of egg layers on substrate $i$ in quadrat $q$,
$\phi_i$ is the proportion of substrate in quadrat $q$ covered by spawn, and
$\rho_i$ is substrate egg density in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$ (\autoref{figUnderDensSubQTS}).
