...
\end{enumerate}
\item Periodically review the accuracy of egg density models using egg layer and vegetation cover estimates compared to egg counts in a subset of sampled quadrats,
...
\begin{enumerate}
\item Update `intensity' from 0 to 1 for the one record in the year 1962, Statistical Area 14, Section 142, Location code 820, and with $\text{intensity} = 0$.
...
\item Update $E_{js}$ to 0.5529 for the four records in the year 1981, Statistical Area 24, and with $E_{js} = 0.0$;
\item Update $E_{js}$ to 1.3360 for the seven records in the year 1982, Statistical Area 23, and with intensity 3;
