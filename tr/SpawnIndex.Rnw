...
This document accompanies our SpawnIndex package (\autoref{secDown}).
The SpawnIndex package has functions to calculate the spawn index,
and is a step towards `good enough' practices in scientific computing \citep{WilsonEtal2016}.
Previously, spawn index calculations were implemented in a \textbf{Microsoft Access} database.
We updated the calculations from a database to an \textbf{R} \citeyearpar[RCT][]{R-4.0.3-32} package for several reasons.
...
$i_{qtxsnry}'$ & Substrate type (understory) & 1, 2, 3, ..., $I_{qtxsnry}'$ \\
...
$s_{nry}=1,\,2,\,3,\,...,\,S_{nry}$ (\autoref{tabNotationIndices}).
...
We refer to the swath of substrate along Macrocystis transects as the transect swath, $\chi_t = 2\,\text{m}$ in transect $t$.
...
B_{s_{nry}}' = \sum_{x=1}^X B_{x_{snry}}',
\end{equation}
