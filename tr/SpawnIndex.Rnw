...
$\female$ & Proportion female & \Sexpr{pars$conversion$phi} & \\
...
$\rho_{iqts}$ is substrate egg density in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ (\autoref{figUnderDensSubQTS}).
...
$\rho_{vqts}$ is in $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ (\autoref{figUnderDensAlgVQTS}).
...
% TODO: They also thank Andy Edwards for help with mathematical notation.
