...
$\zeta$ & Regression slope & \Sexpr{pars$macrocystis$beta}$~\text{eggs} \cdot 10^3 \cdot \text{plant}^{-1}$ & \cite{HaegeleSchweigert1990} \\
$\eta$ & Regression exponent on $\mean{E}$ & \Sexpr{pars$macrocystis$gamma} & \cite{HaegeleSchweigert1990} \\
$\xi$ & Regression exponent on $\mean{H}$ & \Sexpr{pars$macrocystis$delta} & \cite{HaegeleSchweigert1990} \\
...
\mean{\psi_s} = \zeta \mean{E_s}^{\eta} \mean{H_s}^{\xi} \mean{\kappa_s}^{\epsilon} 10^3
\end{equation}
where $\zeta$ is the regression slope,
$\mean{E_s}$ is the mean number of egg layers in spawn $s$ from \autoref{eqMacroLyrsS},
$\eta$ is the regression exponent on $\mean{E_s}$,
$\mean{H_s}$ is the mean plant height in spawn $s$ from \autoref{eqMacroHeightS},
$\xi$ is the regression exponent on $\mean{H_s}$,
...
$\phi^b$ & Proportion of substrate covered in eggs & $0 < \phi^b \leq 1$ & \\
$\lambda$ & Regression slope for substrate & \Sexpr{pars$understory$alpha}$~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{HaegeleEtal1979} \\
$\rho^b$ & Substrate egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$\pi$ & Regression slope for algae & \Sexpr{pars$understory$beta}$~\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \cite{Schweigert2005} \\
$E^v$ & Number of algae egg layers & $E^v > 0$ & \\
$\iota$ & Regression exponent on $E^v$ & \Sexpr{pars$understory$gamma} & \cite{Schweigert2005} \\
...
\rho^b_{qts} = \lambda E^b_{qts} \phi^b_{qts}
\end{equation}
...
\rho^v_{vqts} = \pi E^{v^{\iota}}_{vqts} \phi^{v^{\Delta}}_{vqts} C_v
\end{equation}
where $\pi$ is the regression slope,
$E^v_{vqts}$ is the number of egg layers on algae $v$,
$\iota$ is the regression exponent on $E^v_{vqts}$,
...
$\zeta$, $\eta$, $\xi$, $\epsilon$ & Parameters for number of eggs per Macrocystis plant model (Eq \ref{eqMacroEggsPerPlantS}). & HG in 1981 and 1987 ($n=112$ plants); PRD in 1986 ($n=15$); CC in 1986 ($n=5$); WCVI in 1985 and 1986 ($n=35$). & Model accounts for 78\% of variation in eggs per plant. & \cite{HaegeleSchweigert1990} \\
$\lambda$ & Parameter for substrate egg density model (Eq \ref{eqUnderDensSubQTS}). & SoG in 1976, 1977, and 1978; PRD, CC, and WCVI in 1977. & NA & \cite{HaegeleHumphreys1976, HaegeleHumphreys1978a, HaegeleHumphreys1978b, HaegeleEtal1979} \\
$\pi$, $\iota$, $\Delta$, $C_v$ & Parameters for algae egg density model (Eq \ref{eqUnderDensAlgVQTS}, \autoref{tab:AlgaeCoefs}). & Coastwide from 1976 to 1987 ($n=$ 5,111 samples). & Model accounts for 40\% of variation in egg density. & \cite{Schweigert2005} \\
