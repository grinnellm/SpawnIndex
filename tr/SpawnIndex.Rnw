...
Bien que nous transformions les données de l'enquête sur le frai de la densité des œufs en biomasse en tonnes, la série chronologique qui en résulte est un indice relatif de la biomasse de frai.}
...
throughout coastal BC \citeyearpar[DFO][]{DFO2021}.
...
The overall intent of \fishName{} spawn surveys is to
...
\caption{Spawn index in tonnes (t) by Location in Section \Sexpr{zoom_sec},
Statistical Area \Sexpr{zoom_sa}, \Sexpr{r_name} SAR.}
\label{figRegionSec}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\linewidth}
...
{\fishName{} spawn index by Location in \Sexpr{yr_index} (\subref{figRegionSec}), as well as Sections (\subref{figRegionSA}) and Statistical Areas (\subref{figRegionReg}) in the \Sexpr{reg_name_long} (\Sexpr{r_name}) stock assessment region (SAR; \autoref{figBC}).}
\label{figRegion}
\end{figure}
...
SpawnIndex \textbf{R} package \citeyearpar[RCT][]{R-4.0.5-32} which
...
Each spawn survey type has a different statistical framework
and sampling protocol (\autoref{secProt}):
...
$x$ & Spawn survey type & 1, 2, 3, \ldots, $X_{snry}$ \\
...
(i.e., a function of spawn width), and
...
For a given spawn survey type $x$,
...
In some cases, we adjust spawn width to improve the accuracy
...
Thus, spawn data used for stock assessments is represented either by
...
After estimating the number of eggs in a spawn by spawn survey observation type,
...
(i.e., spawn survey type $x = \text{surface}$).
...
Likewise, we report eggs in thousands (i.e., $10^3 \cdot \text{eggs}$)
in this document and in the \textbf{R} package.}
...
The mean egg density for spawn survey type $x$ in spawn $s$
...
(i.e., spawn survey type $x = \text{Macrocystis}$).
...
The mean width for spawn survey type $x$ in spawn $s$ is
...
The total area of transects for spawn survey type $x$ in spawn $s$ is
...
for spawn survey type $x$ in spawn $s$ is
...
(i.e., spawn survey type $x = \text{understory}$).
...
the mean width of spawn survey type $x$ in spawn $s$ is
...
The mean egg density for spawn survey type $x$ in spawn $s$ is
...
for a given spawn survey type $x$ and spawn $s$
(i.e., $B_{xsnry}'$).
...
in the two spawn survey periods \citeyearpar[DFO][]{DFO2021}.
...
the observed non-stationarity of weight-at-age \citeyearpar[DFO][]{DFO2021}, or
