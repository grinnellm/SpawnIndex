...
\begin{figure}
\centering
\begin{tikzpicture}[node distance=1cm and 0.2cm, font=\footnotesize]
...
\node [inout, below=of mInd, text width=12em] (tIndex) {\labelTotal{tIndex} Total spawn index $B_{s_{nry}}'=\sum_{x_{snry}=1}^{X_{snry}} B_{x_{snry}}'$ (Eq~\ref{eqTotIndSLRY}).};
