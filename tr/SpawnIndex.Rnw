...
\node [inout, below=of mSpawn] (mInd) {\labelMacro{mInd} Macrocystis spawn index $B^\prime_{xs}$ (Eq~\ref{eqMacroIndS}).};
...
\node [inout, below=of sSpawn, left=of mInd] (sInd) {\labelSurf{sInd} Surface spawn index $B^\prime_{xs}$ (Eq~\ref{eqSurfIndS}).};
...
\node [inout, below=of uSpawn, right=of mInd] (uInd) {\labelUnder{uInd} Understory spawn index $B^\prime_{xs}$ (Eq~\ref{eqUnderIndS}).};
\node [inout, below=of mInd] (tIndex) {\labelTotal{tIndex} Total spawn index $B^\prime_s=\sum_{x=1}^X B^\prime_{xs}$ (Eq~\ref{eqTotIndSLRY}).};
...
$L$ & Spawn length & m & \\
$W$ & Spawn width & m & \\
$B^\prime$ & Surface spawn index (i.e., biomass) & t & \\
\bottomrule
\end{tabulary}
...
B^\prime_s = \frac{\mean{\rho_s} L_s W_s 10^3} {\theta}
\end{equation}
...
$B^\prime_s$ is in tonnes (\autoref{figFlow}, step \ref{sInd}).
...
$\mean{\rho}$ & Mean egg density & $\text{eggs} \cdot 10^3 \cdot \text{m}^{-2}$ & \\
$B^\prime$ & Macrocystis spawn index (i.e., biomass) & t & \\
\bottomrule
\end{tabulary}
...
B^\prime_s = \frac{\mean{\rho_s} L^m_s \mean{W_s} 10^3} {\theta}
\end{equation}
...
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B^\prime_s$ is in tonnes (\autoref{figFlow}, step \ref{mInd}).
...
$L$ & Spawn length & m & \\
$B^\prime$ & Understory spawn index (i.e., biomass) & t & \\
\bottomrule
\end{tabulary}
...
B^\prime_s = \frac{\mean{\rho_s} L^v_s \mean{W_s} 10^3} {\theta}
\end{equation}
...
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B^\prime_s$ is in tonnes (\autoref{figFlow}, step \ref{uInd}).
...
B^\prime_{snry} = \sum_{x=1}^X B^\prime_{xsnry}
\end{equation}
where $B^\prime_{xsnry}$ is the spawn index for surface, Macrocystis, and understory spawn surveys from \autoref{eqSurfIndS}, \autoref{eqMacroIndS}, and \autoref{eqUnderIndS}, respectively, and
$B^\prime_{snry}$ is in tonnes (\autoref{figFlow}, step \ref{tIndex}).
...
\textbf{Name} & \textbf{Description} & \textbf{Value or unit} \\
\midrule
$B^\prime$ & Spawn index (i.e., biomass) & t \\
...
B^\prime_{ry} = \sum_{n=1}^N \sum_{s=1}^S B^\prime_{snry}
\end{equation}
where $B^\prime_{snry}$ is the total spawn index from \autoref{eqTotIndSLRY},
and $B^\prime_{ry}$ is a relative index of combined sex spawning biomass for SAR $r$ and year $y$ in tonnes.

\subsection{SCALED ABUNDANCE}

We use $B^\prime_{ry}$ as an indicator of \fishName{} relative population abundance (i.e., biomass) in stock assessment models.
We scale $B^\prime_{ry}$ to abundance
\begin{equation}\label{eqAbundRY}
B_{ry} = \frac{B^\prime_{ry}} {q}
\end{equation}
where $B^\prime_{ry}$ is the total spawn index in tonnes from \autoref{eqTotIndRY},
$q$ is the spawn index scaling parameter \citeyearpar[DFO][]{DFO2020}, and
$B_{ry}$ is scaled abundance (i.e., biomass) for SAR $r$ and year $y$ in tonnes.
Briefly, $q$ accounts \fishName{} eggs lost to predation prior to being measured, and for unobserved \fishName{} spawns \citeyearpar[DFO]{}{DFO2016}.
