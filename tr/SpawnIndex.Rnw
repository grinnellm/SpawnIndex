...
$s_{xnry}=1,\,2,\,3,\,...,\,S_{xnry}$ (\autoref{tabNotationIndices}).
...
The total spawn index in spawn $s$, Location $n$, region $r$, and year $y$
(\autoref{tabNotationIndices}) is
...
We estimate the biomass of \fishName{} that spawned and produced eggs which were removed from the population by SOK fishery $g$ in SAR $r$ and year $y$
(\autoref{tabNotationIndices}) as
