...
\end{equation}
where $B^\prime_{xsnry}$ is spawn index for surface, Macrocystis, and understory spawn surveys from \autoref{eqSurfIndS}, \autoref{eqMacroIndS}, and \autoref{eqUnderIndS}, respectively, and
...
\end{equation}
where $B^\prime_{ry}$ is the spawn index in tonnes from \autoref{eqTotIndRY},
...
Note that we estimate one spawn index scaling parameter for
