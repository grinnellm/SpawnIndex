...
\item Quantify the precision and accuracy of spawn width estimates (e.g., \hyperref[secWidthAdjust]{appendix \ref{secWidthSurface}});
...
In addition to issues regarding uncertainty (\autoref{secUncertain}),
...
\item Surface surveys use two different methods to estimate the number of egg layers:
...
direct estimates (from \Sexpr{directEggYr} to present), and
