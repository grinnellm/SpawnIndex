...
% TODO: Check for instances where a variable has an "xs" subscript,
% but the description says "for spawn s" --
% update the description to say "for spawn type x in spawn s."
% Also check for "for spawn s" in the equation description:
% this is (should be) set up before the equation
% and is not needed in the variable descriptions below the equation.
% Make sure equation variables are described in the order that they appear.
% Make sure all equation outputs (left hand side) have units explained
% (or say that x has no units).
...
\citet{HaegeleEtal1979} developed a model of substrate egg density
in quadrat $q$, transect $t$, and spawn $s$ from egg layers
...
Egg density on algae $v$ in quadrat $q$, transect $t$, and spawn $s$
\citep{Schweigert2005} is
...
$\rho_v$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$
...
The total understory egg density for
quadrat $q$ in transect $t$ and spawn $s$ is
...
\mean{\rho}_{txsnry} = \frac{1}{Q_{txsnry}} \sum_{q=1}^{Q_{txsnry}} \rho_{qtxsnry},
\end{equation}
...
$\mean{\rho}_t$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
...
$\mean{W}_x$ is in metres.
The algae bed length is $L_x''$, also in metres.
As with Macrocystis spawn calculations,
if $L_x''$ is inadvertently not recorded,
we set $L_x''$ to the spawn length $L_x$.
...
where transect egg density is weighted by spawn width at transect $t$, $W_t$.
...
\end{equation}
where $\mean{\rho}_t$ is the mean understory egg density in transect $t$
from \autoref{eqUnderDensTS},
$W_t$ is the spawn width for transect $t$ in metres, and
$\mean{\rho}_x$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.

The understory spawn index in spawn $s$ is
\begin{equation}\label{eqUnderIndS}
B_{xsnry}' = \frac{\mean{\rho}_{xsnry} L_{xsnry}'' \mean{W}_{xsnry} 10^3} {\theta},
\end{equation}
where $\mean{\rho}_x$ is the mean understory egg density from
\autoref{eqUnderDensMuS},
$L_x''$ is the algae bed length,
$\mean{W}_x$ is the mean spawn width from \autoref{eqUnderWidthMuS},
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B_x'$ is in tonnes (step \ref{uInd} in \autoref{figFlow}).
