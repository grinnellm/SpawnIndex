...
This section describes steps \ref{sSamp} to \ref{sInd}
(i.e., spawn survey observation type $x = \text{surface}$; \autoref{figFlow}).
...
Each sample $j$ (\autoref{figFlow}, step \ref{sSamp}) can have
...
This section describes steps \ref{mPlant} to \ref{mInd}
(i.e., spawn survey observation type $x = \text{Macrocystis}$; \autoref{figFlow}).
...
This section describes steps \ref{uQuad} to \ref{uInd}
(i.e., spawn survey observation type $x = \text{understory}$; \autoref{figFlow}).
...
(\autoref{figFlow}, step \ref{uQuad}):
...
At the spawn level (\autoref{figFlow}, step \ref{uSpawn}),
the mean width of spawn $s$ is
...
B_{s_{nry}}' = \frac{\mean{\rho_{s_{nry}}} L_{s_{nry}}'' \mean{W_{s_{nry}}} 10^3} {\theta},
\end{equation}
where $\mean{\rho_s}$ is the mean understory egg density from
...
and $B_{ry}'$ is a relative index of combined sex spawning biomass
