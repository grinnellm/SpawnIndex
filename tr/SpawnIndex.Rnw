...
B^\prime_{ry} = \frac{B_{ry}} {q}
\end{equation}
where
$B_{ry}$ is the spawn index in tonnes from \autoref{eqTotIndRY},
$q$ is the spawn index scaling parameter \citeyearpar[DFO][]{DFO2020}, and
$B^\prime_{ry}$ is scaled abundance for SAR $r$ and year $y$ in tonnes.
...
$B^\prime$ & Scaled abundance (i.e., biomass) & t \\
\bottomrule
\end{tabulary}
...
% TODO: RK - Future spawning biomass! But, those eggs would have had to go through natural mortality and fisheries to be future spawning biomass.
