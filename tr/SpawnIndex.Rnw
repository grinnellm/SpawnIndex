...
\end{equation}
where $E_{ijxsnry}'$ is the number of egg layers on substrate $i$,
...
E_{\cdot jxsnry} = \sum_{i=1}^{I_{jxsnry}} E_{ijxsnry},
\end{equation}
where $E_{ijxsnry}$ is the number of egg layers in substrate $i$
from \autoref{eqSurfLayersIJS}, and
$E_{\cdot jxsnry}$ is in number of egg layers.
...
The mean egg density in spawn $s$ (step \ref{sSpawn} in \autoref{figFlow}) is
\begin{equation}\label{eqSurfDensS}
\mean{\rho}_{xsnry} =
\frac{1}{J_{xsnry}}
\sum_{j=1}^{J_{xsnry}} \rho_{jxsnry},
\end{equation}
where $\rho_{jxsnry}$ is egg density in sample $j$ from \autoref{eqSurfDensJS},
$J_{xsnry}$ is the number of samples, and
$\mean{\rho}_{xsnry}$ is in $10^3 \cdot \text{eggs} \cdot \text{m}^{-2}$.
Two other metrics are required at the spawn level:
spawn length $L$, and
spawn width $W$, both in metres.
We set $W$ to the first non-missing value of
...
$L_{xsnry}$ is length of spawn $s$,
$W_{xsnry}$ is width of spawn $s$,
...
\end{equation}
where $W_t$ is spawn width in metres,
$\chi_t$ is transect swath in metres, and
$A_t$ is in square metres.
...
the Macrocystis bed length is $L_x'$ in metres.
...
\end{equation}
where $\mean{E}_t$ is the mean number of egg layers in transect $t$,
$T$ is the number of transects in spawn $s$, and
$\mean{E}_x$ is in number of egg layers.
...
\end{equation}
where $K_t$ is the number of stalks in transect $t$
from \autoref{eqMacroStalksTS} and
$K_x$ is in number of stalks.
The total number of observed plants in spawn $s$ is
\begin{equation}\label{eqMacroPlantsS}
P_{xsnry} = \sum_{t=1}^{T_{xsnry}} P_{txsnry},
\end{equation}
where $P_t$ is the number of plants in transect $t$ and
$P_x$ is in number of plants.
...
\end{equation}
where $K_x$ is the number of stalks in spawn $s$ from \autoref{eqMacroStalksS},
$P_x$ is the number of plants in spawn $s$ from \autoref{eqMacroPlantsS},
and $\mean{\kappa}_x$ is in number of stalks per plant.
...
$\mean{E}_x$ is the mean number of egg layers in spawn $s$ from \autoref{eqMacroLayersS},
$\gamma$ is the regression exponent on $\mean{E}_x$,
$\mean{H}_x$ is the mean plant height in spawn $s$
from \autoref{eqMacroHeightS},
$\delta$ is the regression exponent on $\mean{H}_x$,
$\mean{\kappa}_x$ is the mean number of stalks per plant in spawn $s$
from \autoref{eqMacroStalksPerPlantS},
$\epsilon$ is the regression exponent on $\mean{\kappa}_x$, and
$\mean{\psi}_x$ is in $\text{eggs} \cdot 10^3 \cdot \text{plant}^{-1}$
...
\end{equation}
where
$\mean{\psi}_x$ is the mean number of eggs per plant in spawn $s$
from \autoref{eqMacroEggsPerPlantS},
$P_x$ is the number of plants in spawn $s$ from \autoref{eqMacroPlantsS},
$A_x$ is the total area of transects in spawn $s$ from \autoref{eqMacroAreaS}, and
...
\end{equation}
where $\mean{\rho}_x$ is the mean egg density in spawn $s$ from \autoref{eqMacroDensS},
$L_x'$ is the length of the Macrocystis bed in spawn $s$,
$\mean{W}_x$ is the mean width of spawn $s$ from \autoref{eqMacroWidthS},
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
$B_x'$ is in tonnes (step \ref{mInd} in \autoref{figFlow}).
...
B_{xsnry}' = \frac{\mean{\rho}_{xsnry} L_{xsnry}'' \mean{W}_{xsnry} 10^3} {\theta},
\end{equation}
where $\mean{\rho_x}$ is the mean understory egg density from
\autoref{eqUnderDensMuS},
$L_x''$ is the length of the algae bed,
$\mean{W_x}$ is the mean spawn width from \autoref{eqUnderWidthMuS},
$\theta$ is the egg conversion factor from \autoref{eqECF}, and
...
Thus, we sum over $x$ to calculate the total spawn index
in spawn $s$ (\autoref{tabNotationIndices})
