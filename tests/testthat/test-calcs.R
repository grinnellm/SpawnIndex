...
test_that("Surface egg density", {
  ...
  expect_silent(egg_dens_surf(
    egg_layers = as.numeric(NA), alpha = as.numeric(NA), beta = as.numeric(NA),
    quiet = TRUE
  ))
})
...
test_that("Macrocystis number of eggs", {
  ...
  expect_error(num_eggs_macro(
    egg_layers = 4, height = 3, stalks_per_plant = 2, xi = "xi"
  ))
  expect_error(num_eggs_macro(
...
test_that("Understory egg density on substrate", {
  ...
  expect_message(egg_dens_under_sub(
    sub_layers = as.numeric(NA), sub_prop = 0.5
  ))
  ...
  expect_error(egg_dens_under_sub(
    sub_layers = 4, sub_prop = 0.5, varphi = "varphi"
  ))
  expect_message(egg_dens_under_sub(
    sub_layers = 4, sub_prop = 0.5, varphi = as.numeric(NA)
  ))
  expect_silent(egg_dens_under_sub(sub_layers = 0, sub_prop = 0, quiet = TRUE))
  expect_silent(egg_dens_under_sub(
...
test_that("Understory egg density on algae", {
  expect_type(egg_dens_under_alg(
    alg_layers = 4, alg_prop = 0.5, coeff = 1.1),
    "double"
  )
  expect_message(egg_dens_under_alg(
    alg_layers = as.numeric(NA), alg_prop = 0.5, coeff = 1.1
  ))
  expect_message(egg_dens_under_alg(
    alg_layers = 0, alg_prop = 0.5, coeff = 1.1)
  )
  expect_message(egg_dens_under_alg(
    alg_layers = 4, alg_prop = as.numeric(NA), coeff = 1.1)
  )
  expect_message(egg_dens_under_alg(alg_layers = 4, alg_prop = 0, coeff = 1.1))
  expect_message(egg_dens_under_alg(
    alg_layers = 4, alg_prop = 1.1, coeff = 1.1)
  )
  expect_message(egg_dens_under_alg(
    alg_layers = 4, alg_prop = 0.5, coeff = as.numeric(NA))
  )
  expect_message(egg_dens_under_alg(
    alg_layers = 4, alg_prop = 0.5, coeff = -0.1)
  )
  expect_error(egg_dens_under_alg(
  ...
  expect_silent(egg_dens_under_alg(
    alg_layers = 0, alg_prop = 0, coeff = -0.1, quiet = TRUE)
  )
  expect_silent(egg_dens_under_alg(
