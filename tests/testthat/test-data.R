test_that("Regions", {
  ...
  expect_equal(dim(regions), c(9, 4))
  expect_named(regions, c("SAR", "Region", "RegionName", "Type"))
  expect_setequal(
    regions$Region,
    c("HG", "PRD", "CC", "SoG", "WCVI", "A27", "A2W", "JS", "A10")
  )
  expect_equal(unique(regions$Type), c("Major", "Minor", "Special"))
})
...
test_that("Understory width factors", {
  ...
  expect_named(
    under_width_facs, c("Year", "HG", "PRD", "CC", "SoG", "WCVI", "A27", "A2W")
  )
  expect_setequal(under_width_facs$Year, 2003:2014)
  expect_setequal(
    unique(unlist(under_width_facs[, 2:8])), c(1.000, 1.075, 1.150)
  )
})
...
test_that("Algae coefficients", {
  ...
  expect_setequal(
    algae_coefs$AlgType, c("GR", "GG", "KF", "KS", "LA", "RW", "SM", "SA")
  )
  expect_setequal(
    algae_coefs$Coef, c(0.9715, 1.000, 0.9119, 1.1766, 0.6553, 0.7793)
  )
})
