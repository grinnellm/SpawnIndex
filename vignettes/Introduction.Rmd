...
areas %>%
  ...
  slice(1:10) %>%
  kable(caption = a_t_cap, booktabs = TRUE, digits = c(0, 0, 0, 0, 0, 3, 3))
```
...
full_join(x = width_bar$region, y = width_bar$section, by = "Region") %>%
  ...
  slice(1:10) %>%
  kable(caption = w_t_cap, booktabs = TRUE, digits = c(0, 1, 0, 1, 0, 1))
```
...
surf_yr <- surf_spawn$si %>%
  ...
  arrange(Year)
# Table
kable(x = surf_yr, digits = c(0, 1), booktabs = TRUE, caption = s_t_cap)
```
...
```{r macro_loc_yr, fig.cap = m_f_cap}
# Figure caption
m_f_cap <- paste(
...
macro_yr <- macro_spawn$si %>%
  ...
  arrange(Year)
# Table
kable(x = macro_yr, digits = c(0, 1), booktabs = TRUE, caption = m_t_cap)
```
...
```{r under_loc_yr, fig.cap = u_f_cap}
# Figure caption
u_f_cap <- paste(
...
under_yr <- under_spawn$si %>%
  ...
  arrange(Year)
# Table
kable(x = under_yr, digits = c(0, 1), booktabs = TRUE, caption = u_t_cap)
```
...
# Table
kable(
  x = total_yr, digits = c(0, 1, 1, 1, 1), booktabs = TRUE, caption = t_t_cap
) %>%
...
# Table
kable(x = kg_sok, digits = c(0, 0, 1), booktabs = TRUE, caption = k_t_cap)
```
