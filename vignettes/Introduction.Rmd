---
title: "Pacific Herring spawn index: An example"
author: "Matthew H. Grinnell"
date: "`r Sys.Date()`"
output:
      rmarkdown::html_vignette:
        fig_caption: yes
bibliography: "../inst/REFERENCES.bib"
link-citations: true
vignette: >
  %\VignetteIndexEntry{Pacific Herring spawn index: An example}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.width = 7,
  fig.height = 7
)
options(knitr.kable.NA = "")
```

This vignette provides an example on how to calculate the spawn index for Pacific Herring (*Clupea pallasii*) in British Columbia, Canada.
See the technical report for background information [@GrinnellEtalYYYY].
Source code is on the [GitHub repository](https://github.com/grinnellm/SpawnIndex).
Load the `SpawnIndex` package and some data for maps.

```{r libraries, message = FALSE, warning = FALSE}
library(SpawnIndex)
library(magrittr) # Pipe %>%
# Black and white theme
ggplot2::theme_set(ggplot2::theme_bw())
# Canada: coarse polygons for mapping
canada <- rnaturalearth::ne_countries(
  scale = "large", returnclass = "sf",
  country = "canada"
)
```

# Set up

Specify the region and years of interest.
Also, load the parameter values required for spawn index calculations.

```{r pars}
# Parameters
region <- "WCVI" # Region: HG, PRD, CC, SoG, WCVI, A27, or A2W
yrRange <- 2010:2015 # Years (>= 1951)
data(pars) # Parameters for spawn index calculations
```

This example will focus on `r region` from `r min(yrRange)` to `r max(yrRange)`.

# Spatial information

Load the spatial information for this region;
we will use the example spawn database that comes with the package (`HerringSpawn.mdb`).
Note that this example database only includes WCVI from 2010 to 2015.

```{r areas}
# Database location
dbLoc <- system.file("extdata", package = "SpawnIndex")
# List indicating the necessary tables: sections and locations
areaLoc <- list(
  loc = dbLoc, db = "HerringSpawn.mdb",
  fns = list(sections = "Sections", locations = "Location")
)
# Load area data
areas <- LoadAreaData(reg = region, where = areaLoc)
# Determine spatial extent (for maps)
xLims <- range(areas$Longitude)
yLims <- range(areas$Latitude)
```

In this example database, `r region` has
`r dplyr::n_distinct(areas$StatArea)` Statistical Areas,
`r dplyr::n_distinct(areas$Section)` Sections, and
`r dplyr::n_distinct(areas$LocationName)` Locations.
Here we show the first 10 Locations.

```{r areasTable, echo = FALSE, results = 'asis'}
areas %>%
  dplyr::select(
    Region, StatArea, Section, LocationName, Longitude,
    Latitude
  ) %>%
  dplyr::arrange(StatArea, Section, LocationName) %>%
  dplyr::slice(1:10) %>%
  knitr::kable(
    caption = paste("Example spawn survey Locations in the", region, "region."),
    booktabs = TRUE
  )
```

# Spawn index calculations

Calculate the conversion factor to convert the number of eggs to the spawn index (i.e., biomass in tonnes).

```{r theta}
# Egg to biomass conversion factor
theta <- CalcEggConversion()
```

Thus, we divide the number of eggs by `r theta` to estimate the biomass of fish that spawned.
This conversion factor is the default in the spawn index calculations below
(e.g., `CalcSurfSpawn()`).
Calculate the spawn index for the three spawn survey types:
surface, Macrocystis, and understory.

## Surface spawn index

Calculate the surface spawn index.
Surface spawn index calculations require
a table to convert intensity categories to the number of egg layers, and
a table of median spawn width estimates.
Median spawn width estimates are required because width estimates from surfaces surveys are inaccurate.

```{r width}
widthLoc <- list(
  loc = dbLoc, db = "HerringSpawn.mdb",
  fns = list(
    regionStd = "RegionStd", sectionStd = "SectionStd", poolStd = "PoolStd"
  )
)
barWidth <- GetWidth(where = widthLoc, a = areas)
```

```{r widthTable, echo = FALSE, results = 'asis'}
barWidth %>%
  dplyr::rename(
    `Region width` = WidthReg, `Section width` = WidthSec,
    `Bed width` = WidthBed
  ) %>%
  dplyr::slice(1:10) %>%
  knitr::kable(
    caption = paste(
      "Example spawn width estimates (m) in the", region, "region."
    ),
    booktabs = TRUE
  )
```

```{r surf}
# Data
data(intensity) # Spawn intensity categories and number of egg layers
# List indicating the necessary tables
surfLoc <- list(
  loc = dbLoc, db = "HerringSpawn.mdb",
  fns = list(surface = "tSSSurface", allSpawn = "tSSAllspawn")
)
# Calculate the surface spawn index
surfSpawn <- CalcSurfSpawn(
  where = surfLoc, a = areas, widths = barWidth, yrs = yrRange
)
```

Show surface spawn on a map, aggregated by Location and year.

```{r surfLocYr, fig.cap = "Surface spawn index in tonnes (t)."}
# Make a spatial object for mapping
surfLocYr <- surfSpawn$SI %>%
  dplyr::group_by(Year, LocationCode) %>%
  dplyr::summarise(Surface = round(sum(SurfSI, na.rm = TRUE))) %>%
  dplyr::ungroup() %>%
  dplyr::left_join(
    y = dplyr::select(.data = areas, LocationCode, Longitude, Latitude),
    by = "LocationCode"
  ) %>%
  sf::st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)
# Map
ggplot2::ggplot(data = surfLocYr) +
  ggplot2::geom_sf(data = canada) +
  ggplot2::geom_sf(mapping = ggplot2::aes(colour = Surface), size = 3) +
  ggplot2::scale_colour_viridis_c() +
  ggplot2::labs(colour = "Spawn\nindex (t)") +
  ggplot2::coord_sf(xlim = xLims, ylim = yLims) +
  ggplot2::facet_wrap(Year ~ ., ncol = 2)
```

Calculate the total surface spawn by year.

```{r surfYr, results = 'asis'}
# Aggregate the surface spawn index by year
surfYr <- surfSpawn$SI %>%
  dplyr::group_by(Year) %>%
  dplyr::summarise(Surface = sum(SurfSI, na.rm = TRUE)) %>%
  dplyr::ungroup() %>%
  tidyr::complete(Year = yrRange) %>%
  dplyr::arrange(Year)
# Table
knitr::kable(
  x = surfYr, digits = 1, caption = "Surface spawn index in tonnes (t) by year.",
  booktabs = TRUE
)
```

## Macrocystis spawn index

Calculate the Macrocystis spawn index.

```{r macro}
# List indicating the necessary tables
macroLoc <- list(
  loc = file.path(dbLoc), db = "HerringSpawn.mdb",
  fns = list(
    allSpawn = "tSSAllspawn", plants = "tSSMacPlant",
    transects = "tSSMacTrans"
  )
)
# Calculate the Macrocystis spawn index
macroSpawn <- CalcMacroSpawn(where = macroLoc, a = areas, yrs = yrRange)
```

Show Macrocystis spawn on a map, aggregated by Location and year.

```{r macroLocYr, fig.cap = "Macrocystis spawn index in tonnes (t)."}
# Make a spatial object for mapping
macroLocYr <- macroSpawn$SI %>%
  dplyr::group_by(Year, LocationCode) %>%
  dplyr::summarise(Macrocystis = round(sum(MacroSI, na.rm = TRUE))) %>%
  dplyr::ungroup() %>%
  dplyr::left_join(
    y = dplyr::select(.data = areas, LocationCode, Longitude, Latitude),
    by = "LocationCode"
  ) %>%
  sf::st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)
# Map
ggplot2::ggplot(data = macroLocYr) +
  ggplot2::geom_sf(data = canada) +
  ggplot2::geom_sf(mapping = ggplot2::aes(colour = Macrocystis), size = 3) +
  ggplot2::scale_colour_viridis_c() +
  ggplot2::labs(colour = "Spawn\nindex (t)") +
  ggplot2::coord_sf(xlim = xLims, ylim = yLims) +
  ggplot2::facet_wrap(Year ~ ., ncol = 2)
```

Calculate the total Macrocystis spawn by year.

```{r macroYr, results = 'asis'}
# Aggregate the Macrocystis spawn index by year
macroYr <- macroSpawn$SI %>%
  dplyr::group_by(Year) %>%
  dplyr::summarise(Macrocystis = sum(MacroSI, na.rm = TRUE)) %>%
  dplyr::ungroup() %>%
  tidyr::complete(Year = yrRange) %>%
  dplyr::arrange(Year)
# Table
knitr::kable(
  x = macroYr, digits = 1,
  caption = "Macrocystis spawn index in tonnes (t) by year.", booktabs = TRUE
)
```

## Understory spawn index

Calculate the understory spawn index.
Understory spawn index calculations require tables to correct spawn widths,
and to specify algae coefficients.

```{r under}
# Data
data(underWidthFac) # Understory spawn width correction factors
data(algaeCoefs) # Algae types and coefficients
# List indicating the necessary tables
underLoc <- list(
  loc = file.path(dbLoc), db = "HerringSpawn.mdb",
  fns = list(
    allSpawn = "tSSAllspawn", algTrans = "tSSVegTrans",
    stations = "tSSStations", algae = "tSSVegetation"
  )
)
# Calculate the understory spawn index
underSpawn <- CalcUnderSpawn(where = underLoc, a = areas, yrs = yrRange)
```

Show understory spawn on a map, aggregated by Location and year.

```{r underLocYr, fig.cap = "Understory spawn index in tonnes (t)."}
# Make a spatial object for mapping
underLocYr <- underSpawn$SI %>%
  dplyr::group_by(Year, LocationCode) %>%
  dplyr::summarise(Understory = round(sum(UnderSI, na.rm = TRUE))) %>%
  dplyr::ungroup() %>%
  dplyr::left_join(
    y = dplyr::select(.data = areas, LocationCode, Longitude, Latitude),
    by = "LocationCode"
  ) %>%
  sf::st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)
# Map
ggplot2::ggplot(data = underLocYr) +
  ggplot2::geom_sf(data = canada) +
  ggplot2::geom_sf(mapping = ggplot2::aes(colour = Understory), size = 3) +
  ggplot2::scale_colour_viridis_c() +
  ggplot2::labs(colour = "Spawn\nindex (t)") +
  ggplot2::coord_sf(xlim = xLims, ylim = yLims) +
  ggplot2::facet_wrap(Year ~ ., ncol = 2)
```

Calculate the total understory spawn by year.

```{r underYr, results = 'asis'}
# Aggregate the understory spawn index by year
underYr <- underSpawn$SI %>%
  dplyr::group_by(Year) %>%
  dplyr::summarise(Understory = sum(UnderSI, na.rm = TRUE)) %>%
  dplyr::ungroup() %>%
  tidyr::complete(Year = yrRange) %>%
  dplyr::arrange(Year)
# Table
knitr::kable(
  x = underYr, digits = 1,
  caption = "Understory spawn index in tonnes (t) by year.", booktabs = TRUE
)
```

# Total spawn index

Calculate the total spawn index by year as the sum of the spawn index in the three survey types:
surface, Macrocystis, and understory.

```{r totalYr, results = 'asis'}
# Total spawn index
totalYr <- dplyr::full_join(x = surfYr, y = macroYr, by = "Year") %>%
  dplyr::full_join(y = underYr, by = "Year") %>%
  dplyr::group_by(Year) %>%
  dplyr::mutate(Total = sum(c(Surface, Macrocystis, Understory),
    na.rm = TRUE
  )) %>%
  dplyr::ungroup() %>%
  tidyr::complete(Year = yrRange) %>%
  dplyr::arrange(Year)
# Table
knitr::kable(
  x = totalYr, digits = 1,
  caption = "Total spawn index in tonnes (t) by year.", booktabs = TRUE
) %>%
  kableExtra::add_header_above(header = c("", "Survey" = 3, ""))
```

# References
