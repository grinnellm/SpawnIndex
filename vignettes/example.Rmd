...
areas <- LoadAreaData(reg = region, where = areaLoc) %>%
  filter(Region == region, Longitude != 0, Latitude != 0)
```
...
# Determine spatial extent (for map)
xSurf <- range(st_coordinates(surfLocYr)[, "X"], na.rm = TRUE)
ySurf <- range(st_coordinates(surfLocYr)[, "Y"], na.rm = TRUE)
# Map
ggplot(data = surfLocYr) +
  geom_sf(data = canada) +
  geom_sf(mapping = aes(colour = Surface), size = 3) +
  scale_colour_viridis_c() +
  labs(colour = "Surface spawn\nindex (t)") +
  coord_sf(xlim = xSurf, ylim = ySurf) +
...
macroLocYr <- macroSpawn$SI %>%
  ...
  st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)
# Determine spatial extent (for map)
xMacro <- range(st_coordinates(macroLocYr)[, "X"], na.rm = TRUE)
yMacro <- range(st_coordinates(macroLocYr)[, "Y"], na.rm = TRUE)
# Map
ggplot(data = macroLocYr) +
  geom_sf(data = canada) +
  geom_sf(mapping = aes(colour = Macrocystis), size = 3) +
  scale_colour_viridis_c() +
  labs(colour = "Macrocystis\nspawn index (t)") +
  coord_sf(xlim = xMacro, ylim = yMacro) +
  facet_wrap(Year ~ ., nrow = 2) +
  theme(legend.position = "top")
```
...
underLocYr <- underSpawn$SI %>%
  ...
  st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)
# Determine spatial extent (for map)
xUnder <- range(st_coordinates(underLocYr)[, "X"], na.rm = TRUE)
yUnder <- range(st_coordinates(underLocYr)[, "Y"], na.rm = TRUE)
# Map
ggplot(data = underLocYr) +
  geom_sf(data = canada) +
  geom_sf(mapping = aes(colour = Understory), size = 3) +
  scale_colour_viridis_c() +
  labs(colour = "Understory\nspawn index (t)") +
  coord_sf(xlim = xUnder, ylim = yUnder) +
  facet_wrap(Year ~ ., nrow = 2) +
  theme(legend.position = "top")
```
